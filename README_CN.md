...
### CPU 动态调度（`index-parallel`）

对于大型多模块项目，`codescope index-parallel` 提供两种调度模式：

**静态模式（默认）：** 按比例预分配——每个模块根据文件数获得固定数量的 worker 核心（`ceil(files × total_workers / total_files)`）。简单、可预测、无共享内存开销。适合中小型项目。

**动态模式（自动启用）：** 共享内存核心池 + 工作窃取——小模块完成释放核心到共享池，待处理的大模块自动领取。适合大型多模块项目。
...
1. **共享内存核心池**（`/tmp/codescope_sched_<pid>.shm`）——固定大小的 `min(host_cpus, total_workers)` 核心池，worker 子进程通过 CAS 原子操作领取/释放。
2. **Worker 队列，大模块优先**——模块按文件数降序排列，大模块最早启动，受益于小模块释放的核心。
3. **核心回收**——小模块完成时释放核心回池；下一个待处理模块领取。这是动态分配相比静态分配的核心优势：已完成小模块的空闲核心被回收给仍在运行的大模块。
4. **内存限制**——调度器每轮轮询采样所有子进程 RSS（50ms 激进模式 / 100ms 普通模式）。若 RSS 超过 `CODESCOPE_MEM_LIMIT_MB`（默认 4096），暂停新 worker 启动，已有 worker 继续运行。

**何时使用哪种模式：**

| 场景 | 推荐 | 原因 |
|------|:----:|------|
| ≤4 模块 或 <1万文件 | **静态** | 动态增加 shm 开销（~300ms）且无回收收益 |
| 5-10 模块，5k-2万文件 | **均可** | 如果 CPU 密集型任务，动态可能有益；建议两者都测试 |
| 10+ 模块，2万+ 文件 | **动态** | 小模块释放核心，大模块吸收 |
| I/O 密集型（SQLite 写入） | **静态** | 瓶颈在磁盘时，更多核心无帮助 |
...
例如，在 14 核机器上：
- 1k 文件 → 5 个 worker（CPU 未充分利用，但避免线程争用）
- 1万文件 → 14 个 worker（每个核心有足够工作）
- 6万文件 → 14 个 worker（I/O 密集型，14 个足够）

**环境变量：**

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `CODESCOPE_DYNAMIC_SCHED` | 自动 | `1`/`true`/`on` = 强制动态，`0`/`false`/`off` = 强制静态 |
| `CODESCOPE_AGGRESSIVE` | `0`（100ms） | `1` = 50ms 轮询间隔（更低延迟，更多 CPU） |
| `CODESCOPE_MEM_LIMIT_MB` | `4096` | RSS 上限（MB）；超过后暂停新 worker 启动 |
| `CODESCOPE_WORKERS` | 自动 | 每个 worker 子进程的解析线程数 |
| `CODESCOPE_SKIP_ASYNC` | `0` | `1` = 跳过异步模型/状态/FTS 构建（用于并行 worker） |

> **注意：** 动态调度的内存监控使用 `pgrep -P <pid>`（来自 `procps-ng`）枚举子进程。macOS 默认可用；Linux 上需确保安装了 `procps-ng`（`apt-get install procps` / `yum install procps-ng`）。如果 `pgrep` 不可用，内存监控静默降级为无操作（无内存限制）。

调度器会向 stderr 打印 `scheduler: [dynamic] ...` 行，显示 `shm_path`、`total_cores`、`mem_limit_mb`、模块分发顺序、核心领取事件和内存压力暂停信息。

### 测试项目基准

所有基准测试在 **Apple M3 Max（14 核，36 GB RAM）** 上运行，使用 `--workers 14 --parallel 4`。

| 项目 | 语言 | 文件数 | 节点数 | 边数 | DB 大小 | 静态耗时 | 动态耗时 | 模块数 | 最佳模式 |
|------|:----:|:-----:|:-----:|:-----:|:-------:|:--------:|:--------:|:-----:|:--------:|
| **CodeScope**（自身） | Rust/C++ | 179 | 1,136 | 1,588 | 2 MB | **1.0s** | 1.4s | 2 | 静态 |
| **memscope-rs** | Rust | 215 | 4,344 | 620 | 1 MB | **1.6s** | 1.9s | 3 | 静态 |
| **Bun** | Rust/C++/TS | 3,177 | 33,312 | 21,654 | 25 MB | **23s** | 28s | 4 | 静态 |
| **Rust 编译器** | Rust | 6,029 | 81,039 | 63,697 | 117 MB | **26s** | 27s | 3 | 静态 |
| **ARES (goagent)** | Go | 1,254 | 18,798 | 4,475 | 8 MB | 6s | **7s** | 7 | 均可 |
| **Linux 内核**（部分） | C/ASM | 27,630 | 988,921 | 864,315 | — | 3分钟+† | 3分钟+† | 20 | **动态** |

† Linux 内核 `drivers` 模块（3.1万文件）因 SQLite I/O 瓶颈未在 300s 内完成。18/20 个模块在约 2.5 分钟内完成。

**关键结论：**

- **小项目（<1k 文件）：** 静态总是更快——动态增加不必要的 shm 开销。
- **中型项目（1k-1万文件，≤4 模块）：** 推荐静态；动态无帮助。
- **中型项目（1k-1万文件，5+ 模块）：** 两者均可；如果 CPU 密集型，动态可能有益。
- **大型项目（1万+文件，10+ 模块）：** 推荐动态模式——核心回收发挥价值。
- **I/O 密集型项目：** 如果 SQLite 写入是瓶颈，无论哪种模式，更多核心都无帮助。
