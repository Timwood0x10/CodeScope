# CodeScope 架构拆解（二）：渐进式就绪——毫秒级让 AI 开始理解你的代码

> 第一次用 CBM 索引 Linux Kernel 的时候，我等了 12 分钟。
> 后来我意识到这不是技术问题，是设计哲学问题。CBM 的模型是全量索引完成后才能查询。CodeScope 的模型是"先让你能用，再逐步加深"。
>
...
| **Linux Kernel** | **\~60,000** | **毫秒级** |

60,000 个文件，毫秒级。这是 Phase A 的核心竞争力——它为 AI 提供了一个"先看看"的入口，成本低到可以忽略不计。
