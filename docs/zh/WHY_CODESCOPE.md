...
> **测试日期**: 2026-07-06（原始对比），2026-07-07（v0.1.0 更新）
...
### 4.0 索引阶段（v0.1.0 重跑基准）

| 指标 | v0.1.1 原始数据 | **v0.1.0 本次实测** |
|------|:---------------:|:-------------------:|
| 索引文件数 | ~200 | **1,116** |
| 索引耗时 | 全量索引中 | **28s** |
...
| 阶段 | codebase-memory-mcp | CodeScope v0.1.1 | **CodeScope v0.1.0 实测** |
|------|:-------------------:|:----------------:|:------------------------:|
...
### 4.3 v0.1.1 → v0.1.0 提升

| 指标 | v0.1.1 | **v0.1.0 实测** | 提升 |
|------|:------:|:---------------:|:----:|
| 查询延迟 | — | **6-7ms** | 🆕 |
...
## v0.1.0 更新：2026-07-07 新增能力

自原始对比以来，CodeScope 在 v0.1.0 中新增了以下能力：

### 🆕 交互式调用链追踪（codescope_trace）

相比原始对比中 CBM 的 `trace_path` 失败、CodeScope 只能查单层 `get_callees`：

| 能力 | CBM v0.8.1 | CodeScope v0.1.1 | CodeScope **v0.1.0** |
|------|:-----------:|:-----------------:|:--------------------:|
| 调用者查询 | ✅ | ✅ | ✅ |
| 被调用者查询 | ✅ | ✅ | ✅ |
| 最短路径 | ❌ 实际测试失败 | ✅ | ✅ |
