...
static void test_symbol_graph_from_semantic_unit()
{
	...
	// Verify nodes
	bool has_compute = false, has_main = false, has_x = false,
	     has_y = false;
	...
	for (auto &e : sym_g.edges) {
		if (e.type == EdgeType::Contains && e.source_id == main_gid &&
		    e.target_id == x_gid)
			contains_x = true;
	}
	assert(contains_x);
...
// ── Test 1b: the anonymous root container is not a node ────────
// Every visitor emits an empty-named Variable at parent_id 0 so each record has
// a parent. It is not a symbol, and the bulk (entity) path never stores it — the
// entity table holds no nameless row — so a single-file graph that keeps it has
// a nameless ghost node the same file's bulk graph does not. The old guard
// (`recordKindToNodeType(kind) == NodeType::File`) could never hold for a
// Variable record, so the container was always kept.
static void test_anonymous_root_is_not_a_node()
{
	using namespace ir;
	using namespace graph;
	SemanticUnit unit;
	SemanticEmitter emitter(&unit);
	emitter.emitVariable("", { 0, 0, 0, 0 }, 0); // anonymous root container
	emitter.emitFunction("solo", { 1, 0, 2, 0 }, 0);

	GraphBuilder builder(1);
	auto g = builder.buildSymbolGraph(unit);
...
static void test_class_method_hierarchy()
{
	...
	for (auto &e : sym_g.edges) {
		if (e.type == EdgeType::Contains && e.source_id == calc_gid &&
		    e.target_id == add_gid)
			contains_add = true;
	}
	assert(contains_add);
...
int main()
{
	...
	test_anonymous_root_is_not_a_node();
