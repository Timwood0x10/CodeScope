...
int main() {
    unlink("/tmp/test_js.db");

    int rc = engine_init("/tmp/test_js.db");
    check(rc == 0, "engine_init");

    // Write a JavaScript test file
...
main();
    ...
    const char* file_path = "/tmp/test_math.js";
    ...
    // Create project
    uint64_t pid = engine_create_project("/tmp", "js-test");
