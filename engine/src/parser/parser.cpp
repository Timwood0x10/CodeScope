...
bool Parser::registerLanguage(const char *name, const char *so_path) {
  ...
  auto *fn =
      reinterpret_cast<const TSLanguage *(*)()>(dlsym(handle, sym.c_str()));
  if (!fn) {
    error_ = std::string("dlsym failed for ") + sym + " in " + so_path + ": " +
             dlerror();
    dlclose(handle);
    return false;
  }
...
TSTree *Parser::parse(const char *file_path, const char *source,
  ...
  TSTree *tree = ts_parser_parse_string(ts_parser, nullptr, source,
