...
bool LspClient::start(const char* command, const char* root_uri) {
    ...
    if (!spawnProcess(command)) return false;
...
std::string LspClient::queryDefinition(const char* file_uri, int line, int column) {
    if (!isRunning()) return "";
    ...
    std::string req = buildJsonRpc("textDocument/definition", params.str(), req_id_);
...
std::string LspClient::readResponse(int expected_id, int timeout_ms) {
    ...
    while (true) {
        ...
        if (timeout_ms > 0) {
            ...
            if (ret <= 0) {
                if (ret == 0) error_ = "timeout waiting for LSP response";
                else error_ = "poll() failed";
                return "";
            }
        }
        ...
        if (header_end != std::string::npos) {
            ...
            if (cl_pos == std::string::npos) continue;

            auto val_start = cl_pos + 15; // after "Content-Length:"
            while (val_start < buffer.size() && buffer[val_start] == ' ') val_start++;
            auto val_end = buffer.find_first_of("\r\n", val_start);
            if (val_end == std::string::npos) continue;

            int content_length = std::atoi(buffer.substr(val_start, val_end - val_start).c_str());
...
std::string LspClient::extractStringField(const std::string& json, const std::string& key) {
