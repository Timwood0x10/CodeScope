...
bool GraphStore::createSchema() {
    ...
    // available) This is optional — if it fails, embeddings just won't be stored.
    {
        ...
        if (rc != SQLITE_OK) {
            ...
            // fallback
            fprintf(stderr, "store: vec0 table not available (sqlite-vec not loaded): %s\n",
                    err ? err : "");
            sqlite3_free(err);
        }
    }
...
uint64_t GraphStore::upsertFile(uint64_t project_id, const char *path, const char *language,
                                const char *content_hash) {
    sqlite3_stmt *stmt = nullptr;
    const char *sql = "INSERT OR REPLACE INTO files (project_id, path, language, "
...
uint64_t GraphStore::insertIRNode(uint64_t project_id, uint64_t file_id, uint64_t parent_id,
                                  int kind, const char *name, const char *qualified_name,
...
bool GraphStore::insertIRSemanticEdge(uint64_t project_id, uint64_t source_id, uint64_t target_id,
                                      int relation) {
    sqlite3_stmt *stmt = nullptr;
...
bool GraphStore::deleteGraphNodesByFile(uint64_t project_id, const char *file_path) {
    ...
    std::ostringstream oss;
    oss << "DELETE FROM graph_nodes WHERE project_id = " << project_id << " AND file_path = '"
        << file_path << "'";
...
uint64_t GraphStore::insertGraphEdge(uint64_t project_id, const graph::GraphEdge &edge) {
    sqlite3_stmt *stmt = nullptr;
    const char *sql = "INSERT INTO graph_edges (project_id, source_node_id, "
...
void GraphStore::insertIntoFTS(uint64_t node_id, uint64_t project_id, const char *name,
                               const char *qualified_name, const char *file_path,
                               const char *content, int node_kind) {
    // Skip empty entries
    if ((!name || !*name) && (!qualified_name || !*qualified_name) && (!file_path || !*file_path) &&
        (!content || !*content)) {
        return;
    }
    if (node_kind < 0)
...
bool GraphStore::setComplexity(uint64_t project_id, uint64_t graph_node_id, uint64_t cyclomatic,
                               uint64_t cognitive, uint64_t nesting_depth,
                               uint64_t decision_points) {
    sqlite3_stmt *stmt = nullptr;
...
std::string GraphStore::getComplexityJson(uint64_t project_id, uint64_t graph_node_id) {
    sqlite3_stmt *stmt = nullptr;
    const char *sql = "SELECT nc.cyclomatic, nc.cognitive, nc.nesting_depth, "
                      "nc.decision_points, "
...
bool GraphStore::storeVector(uint64_t node_id, uint64_t project_id, const void *vec_data,
                             size_t vec_bytes) {
    sqlite3_stmt *stmt = nullptr;
    const char *sql = "INSERT OR REPLACE INTO node_vectors (node_id, project_id, "
                      "vector) VALUES (?, ?, ?)";
    sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    sqlite3_bind_int64(stmt, 1, static_cast<int64_t>(node_id));
    sqlite3_bind_int64(stmt, 2, static_cast<int64_t>(project_id));
...
std::string GraphStore::searchSemantic(uint64_t project_id, const void *query_vec, size_t vec_bytes,
                                       int limit) {
...
uint64_t GraphStore::insertModule(uint64_t project_id, uint64_t parent_id, const char *name,
...
uint64_t GraphStore::insertSymbol(uint64_t project_id, uint64_t module_id, const char *kind,
                                  const char *name, const char *signature, const char *visibility,
                                  const char *language, const char *file_path, int line, int column,
    ...
    {
        const char *ss_sql = "INSERT OR IGNORE INTO symbol_status (symbol_id) VALUES (?)";
        sqlite3_stmt *ss_stmt = nullptr;
        if (sqlite3_prepare_v2(db_, ss_sql, -1, &ss_stmt, nullptr) == SQLITE_OK) {
            sqlite3_bind_int64(ss_stmt, 1, static_cast<int64_t>(id));
...
bool GraphStore::insertEntryPoint(uint64_t symbol_id, uint64_t project_id, const char *kind) {
...
uint64_t GraphStore::insertCallEdge(uint64_t project_id, uint64_t caller_symbol_id,
                                    uint64_t callee_symbol_id, const char *provenance, int line,
                                    int col) {
...
uint64_t GraphStore::insertDependencyEdge(uint64_t project_id, uint64_t source_module_id,
                                          uint64_t target_module_id, const char *external_name,
                                          const char *kind) {
...
bool GraphStore::insertMetric(uint64_t project_id, const char *owner_type, uint64_t owner_id,
                              int cyclomatic, int nesting_depth, int cognitive, int lines,
                              int param_count, int call_count, int branch_count, int loop_count) {
...
void GraphStore::insertIntoSearchIndex(uint64_t symbol_id, uint64_t project_id, const char *title,
...
bool GraphStore::insertEmbedding(uint64_t symbol_id, const float *vector_data, int dim) {
    ...
    sqlite3_bind_int64(stmt, 1, static_cast<int64_t>(symbol_id));
    sqlite3_bind_blob(stmt, 2, vec.data(), TARGET_DIM * static_cast<int>(sizeof(float)),
                      SQLITE_TRANSIENT);
    if (sqlite3_step(stmt) != SQLITE_DONE) {
...
bool GraphStore::setSymbolReady(uint64_t symbol_id, const char *field) {
    std::string sql = "UPDATE symbol_status SET " + std::string(field) + " = 1 WHERE symbol_id = ?";
    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(db_, sql.c_str(), -1, &stmt, nullptr) != SQLITE_OK) {
...
std::vector<std::string> GraphStore::getUnreadyFiles(uint64_t project_id, const char *ready_field) {
...
uint64_t GraphStore::createTask(uint64_t project_id, const char *task_type) {
    const char *sql = "INSERT INTO index_tasks (project_id, task_type) VALUES (?, ?)";
    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) != SQLITE_OK) {
...
bool GraphStore::updateTask(uint64_t task_id, const char *status, int progress, const char *error) {
    std::string sql = "UPDATE index_tasks SET status = ?, progress = ?, error = ?";
...
std::string GraphStore::getTaskStatusJson(uint64_t project_id) {
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        const char *tt = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 1));
        const char *st = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 2));
        int pr = sqlite3_column_int(stmt, 3);
        const char *er = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 4));
        const char *sa = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 5));
...
std::string GraphStore::findCallersJson(uint64_t project_id, const char *symbol_name) {
    ...
    const char *sql = "SELECT DISTINCT caller.id, caller.name, caller.kind, "
...
std::string GraphStore::findCalleesJson(uint64_t project_id, const char *symbol_name) {
    const char *sql = "SELECT DISTINCT callee.id, callee.name, callee.kind, "
...
std::string GraphStore::tracePathJson(uint64_t project_id, const char *from_name,
                                      const char *to_name) {
    // 1. Find symbol IDs
