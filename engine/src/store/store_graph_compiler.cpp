                            ...
                            uint64_t project_id)
{
    ...
    std::string uid = makeNodeUid(
        project_id, sqliteText(st, 13),
...
bool compileGraphToLadybugDB(GraphStore *store, uint64_t project_id)
{
    ...
    // ── Step 1: Clear existing subgraph for this project ──
    {
        std::string clear = "MATCH (n:GraphNode {project_id:" + pid +
                            "}) DETACH DELETE n";
        lbug_query_result qr;
        lbug_state state = lbug_connection_query(conn, clear.c_str(), &qr);
        if (state != LbugSuccess) {
...
FROM graph_nodes WHERE project_id = ? ORDER BY id)";
        ...
        while (sqlite3_step(st) == SQLITE_ROW) {
            ...
            if (n % kCompileBatchSize == 0 &&
                !runNodeBatch(conn, buf, n, "compileGraphToLadybugDB")) {
                sqlite3_finalize(st);
                return false;
            }
        }
...
WHERE e.project_id = ? ORDER BY e.id)";
        ...
        while (sqlite3_step(st) == SQLITE_ROW) {
            if (sqlite3_column_int(st, 4) == 3) {
                appendEdgeEntry(relates_buf, relates_n, st, project_id);
                if (relates_n % kCompileBatchSize == 0 &&
                    !runEdgeBatch(conn, relates_buf, relates_n, project_id,
                                  true, "compileGraphToLadybugDB")) {
                    sqlite3_finalize(st);
                    return false;
                }
            } else {
                appendEdgeEntry(calls_buf, calls_n, st, project_id);
                if (calls_n % kCompileBatchSize == 0 &&
                    !runEdgeBatch(conn, calls_buf, calls_n, project_id,
                                  false, "compileGraphToLadybugDB")) {
                    sqlite3_finalize(st);
                    return false;
                }
            }
        ...
        if (!runEdgeBatch(conn, relates_buf, relates_n, project_id, true,
                          "compileGraphToLadybugDB")) {
            return false;
        }
    }
...
bool compileGraphToLadybugDB(GraphStore * /*store*/,
                             uint64_t /*project_id*/)
{
