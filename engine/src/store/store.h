...
class GraphStore {
  public:
    ...
    uint64_t upsertFile(uint64_t project_id, const char *path, const char *language,
                        const char *content_hash);

    // ── IR Nodes ───────────────────────────────────────────────

    uint64_t insertIRNode(uint64_t project_id, uint64_t file_id, uint64_t parent_id, int kind,
                          const char *name, const char *qualified_name, uint32_t sr, uint32_t sc,
                          uint32_t er, uint32_t ec, const char *language);

    bool insertIRSemanticEdge(uint64_t project_id, uint64_t source_id, uint64_t target_id,
                              int relation);
    ...
    void insertIntoFTS(uint64_t node_id, uint64_t project_id, const char *name,
                       const char *qualified_name, const char *file_path, const char *content,
                       int node_kind = -1);

    // ── Vector search (semantic) ─────────────────────────────────

    bool storeVector(uint64_t node_id, uint64_t project_id, const void *vec_data, size_t vec_bytes);
    std::string searchSemantic(uint64_t project_id, const void *query_vec, size_t vec_bytes,
                               int limit);
    void deleteFTSByFile(uint64_t project_id, uint64_t file_id);

    // ── Complexity ───────────────────────────────────────────────

    bool setComplexity(uint64_t project_id, uint64_t graph_node_id, uint64_t cyclomatic,
                       uint64_t cognitive, uint64_t nesting_depth, uint64_t decision_points);
     ...
     */
    uint64_t insertModule(uint64_t project_id, uint64_t parent_id, const char *name,
     ...
     */
    uint64_t insertSymbol(uint64_t project_id, uint64_t module_id, const char *kind,
    ...
    bool insertEntryPoint(uint64_t symbol_id, uint64_t project_id, const char *kind);
    ...
    uint64_t insertCallEdge(uint64_t project_id, uint64_t caller_symbol_id,
                            uint64_t callee_symbol_id, const char *provenance, int line, int col);
    ...
    bool insertMetric(uint64_t project_id, const char *owner_type, uint64_t owner_id,
                      int cyclomatic, int nesting_depth, int cognitive, int lines, int param_count,
                      int call_count, int branch_count, int loop_count);

    // ── Phase B: Enhancement — Search Index ───────────────────

    void insertIntoSearchIndex(uint64_t symbol_id, uint64_t project_id, const char *title,
    ...
    bool isFileUnchanged(uint64_t project_id, const char *file_path, int64_t mtime, int64_t size);
    void updateFileScanState(uint64_t project_id, const char *file_path, int64_t mtime, int64_t size);
    void cleanupStaleFiles(uint64_t project_id, const std::vector<std::string> &active_files);
     ...
     */
    std::string tracePathJson(uint64_t project_id, const char *from_name, const char *to_name);
    ...
    bool updateTask(uint64_t task_id, const char *status, int progress, const char *error);
