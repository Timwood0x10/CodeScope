...
TranslationUnit *GoTranslator::translate(TSTree *tree, const char *source, const char *file_path) {
...
Node *GoTranslator::translateNode(TSNode ts_node, Node *parent) {
    ...
    if (strcmp(type, "method_declaration") == 0)
        return handleMethodDecl(ts_node, parent);
    if (strcmp(type, "type_declaration") == 0)
        return handleTypeDecl(ts_node, parent);
    if (strcmp(type, "call_expression") == 0)
        return handleCallExpr(ts_node, parent);
    if (strcmp(type, "selector_expression") == 0)
        return handleSelectorExpr(ts_node, parent);
    ...
    if (strcmp(type, "const_declaration") == 0)
        return handleVarDecl(ts_node, parent);
    if (strcmp(type, "short_var_declaration") == 0)
        return handleShortVarDecl(ts_node, parent);
    ...
    if (strcmp(type, "expression_statement") == 0 || strcmp(type, "statement_list") == 0 ||
        strcmp(type, "block") == 0 || strcmp(type, "expression_list") == 0 ||
        strcmp(type, "argument_list") == 0 || strcmp(type, "parameter_list") == 0 ||
        strcmp(type, "literal_value") == 0 || strcmp(type, "composite_literal") == 0 ||
        strcmp(type, "binary_expression") == 0 || strcmp(type, "unary_expression") == 0 ||
...
Node *GoTranslator::handleFuncDecl(TSNode ts_node, Node *parent) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (strcmp(ts_node_type(child), "identifier") == 0)
            continue;
        if (strcmp(ts_node_type(child), "func") == 0)
            continue;
