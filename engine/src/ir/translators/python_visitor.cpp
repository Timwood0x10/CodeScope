...
SemanticUnit *PythonVisitor::visit(TSTree *tree, const char *source, const char *fp) {
    SemanticUnit *unit = JsVisitor::visit(tree, source, fp);
    if (unit) unit->setLanguage("python");
    return unit;
}
void PythonVisitor::visitNode(TSNode node, uint64_t parent_id) {
    const char *type = ts_node_type(node);
    if (strcmp(type, "function_definition") == 0) return handleFuncDef(node, parent_id);
    if (strcmp(type, "class_definition") == 0) return handleClassDef(node, parent_id);
    if (strcmp(type, "call") == 0) return handleCall(node, parent_id);
    if (strcmp(type, "import_statement") == 0 || strcmp(type, "import_from_statement") == 0)
        return handleImport(node, parent_id);
    if (strcmp(type, "assignment") == 0) return handleAssignment(node, parent_id);
...
void PythonVisitor::handleClassDef(TSNode node, uint64_t parent_id) {
    ...
    for (uint32_t i = 0; i < cnt; i++) {
        ...
        const char *t = ts_node_type(c);
        if (strcmp(t, "identifier") == 0) continue;
        if (strcmp(t, "block") == 0) visitChildren(c, id);
    }
    popScope();
}
