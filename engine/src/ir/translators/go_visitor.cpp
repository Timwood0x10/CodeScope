...
void GoVisitor::visitNode(TSNode node, uint64_t parent_id) {
    ...
    if (strcmp(type, "type_declaration") == 0) return handleTypeDecl(node, parent_id);
    ...
    if (strcmp(type, "var_declaration") == 0 || strcmp(type, "const_declaration") == 0) return handleVarDecl(node, parent_id);
    if (strcmp(type, "short_var_declaration") == 0) return handleShortVar(node, parent_id);
...
void GoVisitor::handleMethodDecl(TSNode node, uint64_t parent_id) {
    ...
    for (uint32_t i = 0; i < cnt; i++) {
        ...
        const char *t = ts_node_type(c);
        if (strcmp(t, "parameter_list") == 0 || strcmp(t, "block") == 0 ||
            strcmp(t, "type_identifier") == 0) continue;
        visitChildren(c, id);
    }
    popScope();
}
...
void GoVisitor::handleCall(TSNode node, uint64_t parent_id) {
    ...
    for (uint32_t i = 0; i < cnt; i++) {
        TSNode c = ts_node_child(node, i);
        if (!ts_node_is_named(c)) continue;
        if (strcmp(ts_node_type(c), "identifier") == 0) { name = nodeText(c); break; }
    }
