...
SemanticUnit *RustVisitor::visit(TSTree *tree, const char *source, const char *fp) {
    SemanticUnit *unit = JsVisitor::visit(tree, source, fp);
    if (unit) unit->setLanguage("rust");
    return unit;
}
void RustVisitor::visitNode(TSNode node, uint64_t parent_id) {
    const char *type = ts_node_type(node);
    if (strcmp(type, "function_item") == 0) return handleFunction(node, parent_id);
    if (strcmp(type, "struct_item") == 0 || strcmp(type, "union_item") == 0)
        return handleStruct(node, parent_id);
    if (strcmp(type, "enum_item") == 0) return handleEnum(node, parent_id);
    if (strcmp(type, "trait_item") == 0) return handleTrait(node, parent_id);
    if (strcmp(type, "impl_item") == 0) return handleImpl(node, parent_id);
    if (strcmp(type, "call_expression") == 0) return handleCall(node, parent_id);
    if (strcmp(type, "let_declaration") == 0) return handleLet(node, parent_id);
    if (strcmp(type, "use_declaration") == 0) return handleUse(node, parent_id);
        ...
        if (strcmp(ts_node_type(c), "function_item") == 0) {
            ...
            if (!name.empty()) {
                ...
                for (uint32_t j = 0; j < cc; j++) {
                    ...
                    if (!ts_node_is_named(gc)) continue;
