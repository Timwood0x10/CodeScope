...
Node* TypescriptTranslator::translateNode(TSNode ts_node, Node* parent) {
    ...
    // TypeScript-specific
    if (strcmp(type, "interface_declaration") == 0)     return handleInterface(ts_node, parent);
    if (strcmp(type, "type_alias_declaration") == 0)    return handleTypeAlias(ts_node, parent);
    if (strcmp(type, "enum_declaration") == 0)          return handleEnum(ts_node, parent);

    // JavaScript-shared
    if (strcmp(type, "function_declaration") == 0)         return handleFunction(ts_node, parent);
    if (strcmp(type, "arrow_function") == 0)               return handleArrow(ts_node, parent);
    if (strcmp(type, "generator_function_declaration") == 0) return handleFunction(ts_node, parent);
    if (strcmp(type, "class_declaration") == 0)            return handleClass(ts_node, parent);
    if (strcmp(type, "method_definition") == 0)            return handleMethod(ts_node, parent);
    if (strcmp(type, "call_expression") == 0)              return handleCall(ts_node, parent);
    if (strcmp(type, "identifier") == 0)                   return handleIdentifier(ts_node, parent);
    if (strcmp(type, "variable_declaration") == 0)         return handleVariable(ts_node, parent);
    if (strcmp(type, "lexical_declaration") == 0)          return handleVariable(ts_node, parent);
    if (strcmp(type, "import_statement") == 0 ||
        strcmp(type, "import") == 0)                       return handleImport(ts_node, parent);
    if (strcmp(type, "export_statement") == 0 ||
        strcmp(type, "export") == 0)                       return handleExport(ts_node, parent);
    if (strcmp(type, "member_expression") == 0)            return handleMember(ts_node, parent);
    ...
    if (strcmp(type, "expression_statement") == 0 ||
        ...
        strcmp(type, "literal_type") == 0 ||
        strcmp(type, "union_type") == 0 ||
        strcmp(type, "intersection_type") == 0 ||
        strcmp(type, "array_type") == 0 ||
        strcmp(type, "tuple_type") == 0 ||
        strcmp(type, "function_type") == 0 ||
        strcmp(type, "conditional_type") == 0 ||
        strcmp(type, "mapped_type") == 0 ||
        strcmp(type, "index_type_query") == 0 ||
        ...
        strcmp(type, "call_signature") == 0 ||
        strcmp(type, "construct_signature") == 0 ||
        strcmp(type, "index_signature") == 0 ||
        strcmp(type, "property_signature") == 0 ||
        strcmp(type, "decorator") == 0 ||
        strcmp(type, "implements_clause") == 0 ||
        strcmp(type, "extends_clause") == 0 ||
        ...
        strcmp(type, "accessibility_modifier") == 0 ||
...
Node* TypescriptTranslator::handleFunction(TSNode ts_node, Node* parent) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (strcmp(t, "identifier") == 0) continue;
        if (strcmp(t, "formal_parameters") == 0) translateChildren(child, func);
        else if (strcmp(t, "statement_block") == 0) translateChildren(child, func);
    }
    popScope();
    return func;
}
...
Node* TypescriptTranslator::handleMethod(TSNode ts_node, Node* parent) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        const char* t = ts_node_type(child);
        if (strcmp(t, "property_identifier") == 0 ||
            strcmp(t, "shorthand_property_identifier") == 0) continue;
        if (strcmp(t, "formal_parameters") == 0) translateChildren(child, method);
        else if (strcmp(t, "statement_block") == 0) translateChildren(child, method);
    }
    popScope();
    return method;
}
...
Node* TypescriptTranslator::handleCall(TSNode ts_node, Node* parent) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (strcmp(t, "identifier") == 0) {
            ...
            if (target) call->semantic_edges.push_back({target, Relation::CallTarget});
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (strcmp(ts_node_type(child), "variable_declarator") == 0) {
            ...
            if (var) {
                for (uint32_t j = 0; j < dc; j++) {
                    TSNode decl = ts_node_child(child, j);
                    if (strcmp(ts_node_type(decl), "identifier") == 0) continue;
                    if (ts_node_is_named(decl)) translateNode(decl, var);
                }
            } else {
