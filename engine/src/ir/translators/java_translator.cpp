...
Node* JavaTranslator::translateNode(TSNode ts_node, Node* parent) {
    ...
    if (strcmp(type, "method_declaration") == 0)          return handleMethod(ts_node, parent);
    if (strcmp(type, "constructor_declaration") == 0)     return handleConstructor(ts_node, parent);
    if (strcmp(type, "method_invocation") == 0)           return handleCall(ts_node, parent);
    if (strcmp(type, "identifier") == 0)                  return handleIdentifier(ts_node, parent);
    if (strcmp(type, "field_declaration") == 0)           return handleField(ts_node, parent);
    if (strcmp(type, "variable_declaration") == 0 ||
        strcmp(type, "local_variable_declaration") == 0 ||
        strcmp(type, "enhanced_for_control") == 0)        return handleVarDecl(ts_node, parent);
    if (strcmp(type, "import_declaration") == 0)          return handleImport(ts_node, parent);
    if (strcmp(type, "object_creation_expression") == 0)  return handleNew(ts_node, parent);
    ...
    if (strcmp(type, "expression_statement") == 0 ||
        ...
        strcmp(type, "modifier") == 0 ||
        strcmp(type, "annotation") == 0 ||
