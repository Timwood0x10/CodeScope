...
Node* CTranslator::translateNode(TSNode ts_node, Node* parent) {
    ...
    if (strcmp(type, "struct_specifier") == 0)    return handleStruct(ts_node, parent, false);
    if (strcmp(type, "union_specifier") == 0)     return handleStruct(ts_node, parent, true);
    if (strcmp(type, "enum_specifier") == 0) {
        auto* en = makeNode(NodeKind::EnumDecl, ts_node);
        std::string name = extractName(ts_node);
        en->name = name;
        if (!name.empty()) defineSymbol(name, en);
