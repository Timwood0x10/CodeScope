...
TranslationUnit *CTranslator::translate(TSTree *tree, const char *source, const char *file_path) {
...
Node *CTranslator::translateNode(TSNode ts_node, Node *parent) {
    ...
    if (strcmp(type, "expression_statement") == 0 || strcmp(type, "compound_statement") == 0 ||
        strcmp(type, "enumerator_list") == 0 || strcmp(type, "argument_list") == 0 ||
        strcmp(type, "expression_statement") == 0 || strcmp(type, "parameter_list") == 0 ||
        strcmp(type, "init_declarator") == 0 || strcmp(type, "pointer_declarator") == 0 ||
        strcmp(type, "function_declarator") == 0 || strcmp(type, "array_declarator") == 0 ||
        strcmp(type, "parenthesized_declarator") == 0 ||
        strcmp(type, "field_declaration_list") == 0 || strcmp(type, "field_declaration") == 0) {
...
Node *CTranslator::handleCallExpr(TSNode ts_node, Node *parent) {
    ...
    fprintf(stderr, "CALL_DBG: kind=%d name='%s' parent_kind=%d\n", (int)call->kind,
            call->name.c_str(), parent ? static_cast<int>(parent->kind) : -1);
