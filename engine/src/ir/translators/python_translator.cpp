...
void PythonTranslator::pushScope() { scopes_.push_back(Scope{}); }
...
void PythonTranslator::defineSymbol(const std::string &name, Node *node) {
  ...
  definitions_[name] =
      node; // flat global lookup for simplicity (v2: qualified names)
}
...
Node *PythonTranslator::translateNode(TSNode ts_node, Node *parent) {
  ...
  if (strcmp(type, "assignment") == 0 ||
      strcmp(type, "augmented_assignment") == 0)
  ...
  if (strcmp(type, "expression_statement") == 0 ||
      strcmp(type, "decorated_definition") == 0 || strcmp(type, "block") == 0) {
  ...
  // Fallback: generic block/statement — skip for now, children handled by
  // parent
  return nullptr;
}
...
Node *PythonTranslator::handleFunctionDef(TSNode ts_node, Node *parent) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    if (!ts_node_is_named(child))
      continue;
    if (strcmp(ts_node_type(child), "identifier") == 0)
      continue; // name already extracted
...
Translator *createPythonTranslator() { return new PythonTranslator(); }
