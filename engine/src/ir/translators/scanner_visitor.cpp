...
SemanticUnit *ScannerVisitor::scan(const std::string &source,
    ...
    while (std::getline(stream, line)) {
        ...
        uint32_t end_col = static_cast<uint32_t>(
            sv.size() > 80 ? 80 : sv.size());
        SourceRange loc{
            static_cast<uint32_t>(line_num - 1), 0,
...
void ScannerVisitor::emitDecl(const std::string &kind,
...
bool ScannerVisitor::startsWithKW(std::string_view line, const char *kw) {
    ...
    if (klen > 0 && kw[klen - 1] == ' ') return true;
...
std::string ScannerVisitor::detectDeclLine(std::string_view line) {
    ...
    if (language_ == "c" || language_ == "cpp") {
        ...
        if (line.find('(') != std::string_view::npos &&
            ...
            auto name_tok = line.substr(name_start, name_end - name_start);
            for (const char **s = skip; *s; s++)
                if (name_tok == *s) return "";
            // Must have a known type keyword before the name
            auto before = trimLeft(line.substr(0, name_start));
            static const char *types[] = {"int ", "void ", "char ", "long ",
                "short ", "float ", "double ", "bool ", "signed ", "unsigned ",
                "const ", "struct ", "union ", "enum ", "class ",
                "size_t ", "ssize_t ", nullptr};
    ...
    // Rust
    if (language_ == "rust") {
        if (startsWithKW(line, "pub unsafe fn ") || startsWithKW(line, "pub async fn ") ||
            startsWithKW(line, "pub fn ") || startsWithKW(line, "fn "))
            return "function";
        if (startsWithKW(line, "pub struct ") || startsWithKW(line, "struct "))
            return "struct";
        if (startsWithKW(line, "pub enum ") || startsWithKW(line, "enum "))
            return "enum";
        if (startsWithKW(line, "pub trait ") || startsWithKW(line, "trait "))
            return "interface";
        if (startsWithKW(line, "pub type ") || startsWithKW(line, "type "))
            return "type_alias";
    }

    // Python
    if (language_ == "python") {
        if (startsWithKW(line, "async def ") || startsWithKW(line, "def "))
            return "function";
        if (startsWithKW(line, "class ")) return "class";
    }
    ...
    if (language_ == "javascript" || language_ == "typescript") {
        if (startsWithKW(line, "export async function ") ||
            ...
            startsWithKW(line, "function ")) return "function";
        if (startsWithKW(line, "export class ") || startsWithKW(line, "class "))
            return "class";
        if (startsWithKW(line, "export interface ") || startsWithKW(line, "interface "))
            return "interface";
        if (startsWithKW(line, "export enum ") || startsWithKW(line, "enum "))
            return "enum";
    }
    ...
    // Java
    if (language_ == "java") {
        if (startsWithKW(line, "class ") || startsWithKW(line, "public class ") ||
            startsWithKW(line, "private class ") || startsWithKW(line, "protected class "))
            return "class";
        if (startsWithKW(line, "interface ") || startsWithKW(line, "public interface "))
            return "interface";
        if (startsWithKW(line, "enum ") || startsWithKW(line, "public enum "))
            return "enum";
    }

    // Swift
    if (language_ == "swift") {
        if (startsWithKW(line, "func ") || startsWithKW(line, "public func ") ||
            startsWithKW(line, "open func "))
            return "function";
        if (startsWithKW(line, "class ") || startsWithKW(line, "public class ") ||
            startsWithKW(line, "open class "))
            return "class";
        if (startsWithKW(line, "struct ") || startsWithKW(line, "public struct "))
            return "struct";
        if (startsWithKW(line, "enum ") || startsWithKW(line, "public enum "))
            return "enum";
        if (startsWithKW(line, "protocol ") || startsWithKW(line, "public protocol "))
            return "interface";
    }
...
std::string ScannerVisitor::extractDeclName(std::string_view line,
    ...
    if (kind == "struct" || kind == "class" || kind == "enum" ||
        ...
        if (language_ == "rust") {
            static const char *rk[] = {"pub struct", "struct", "pub enum", "enum",
                "pub trait", "trait", "pub type", "type",
                "pub union", "union", nullptr};
            skip_prefix(rk);
        } else if (language_ == "c" || language_ == "cpp") {
            static const char *ck[] = {"class ", "struct ", "enum ", "union ",
                "enum class ", nullptr};
            skip_prefix(ck);
        } else if (language_ == "java") {
            static const char *jk[] = {"public class", "private class",
                "protected class", "class", "public interface", "interface",
                "public enum", "enum", nullptr};
            skip_prefix(jk);
        } else if (language_ == "swift") {
            static const char *sk[] = {"open class", "public class", "class",
                "public struct", "struct", "public enum", "enum",
                "public protocol", "protocol", nullptr};
            skip_prefix(sk);
        } else if (language_ == "go") {
            if (line.substr(0, 5) == "type ") line = trimLeft(line.substr(5));
        } else if (language_ == "python") {
            static const char *pk[] = {"class", nullptr};
            skip_prefix(pk);
        } else if (language_ == "javascript" || language_ == "typescript") {
            static const char *jk2[] = {"export class", "class", "export interface",
                "interface", "export enum", "enum", nullptr};
            skip_prefix(jk2);
        }
    ...
    if (language_ == "rust") {
        static const char *rk[] = {"pub unsafe fn", "pub async fn", "pub fn",
            "fn", "pub struct", "struct", "pub enum", "enum",
            "pub trait", "trait", "pub type", "type", nullptr};
        skip_prefix(rk);
    } else if (language_ == "python") {
        static const char *pk[] = {"async def", "def", "class", nullptr};
        skip_prefix(pk);
    } else if (language_ == "javascript" || language_ == "typescript") {
        static const char *jk[] = {"export async function", "export function",
            "async function", "function", "export class", "class",
            "export interface", "interface", "export enum", "enum", nullptr};
        skip_prefix(jk);
    } else if (language_ == "go") {
        if (line.substr(0, 5) == "func ") line = trimLeft(line.substr(5));
        else if (line.substr(0, 5) == "type ") line = trimLeft(line.substr(5));
    } else if (language_ == "swift") {
        static const char *sk[] = {"open func", "public func", "func",
            "open class", "public class", "class",
            "public struct", "struct", "public enum", "enum",
            "public protocol", "protocol", nullptr};
        skip_prefix(sk);
    } else if (language_ == "java") {
        static const char *jk2[] = {"public class", "private class",
            "protected class", "class", "public interface", "interface",
            "public enum", "enum", nullptr};
        skip_prefix(jk2);
    } else if (language_ == "c" || language_ == "cpp") {
        // For C/C++, skip storage class or type prefix, then find name before '('
        static const char *ck[] = {"static ", "extern ", "inline ",
            "virtual ", "constexpr ", "const ", "volatile ", nullptr};
        skip_prefix(ck);
