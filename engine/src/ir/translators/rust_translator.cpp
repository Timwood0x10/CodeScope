...
Node* RustTranslator::translateNode(TSNode ts_node, Node* parent) {
    ...
    if (strcmp(type, "function_item") == 0)           return handleFunction(ts_node, parent);
    if (strcmp(type, "struct_item") == 0)             return handleStruct(ts_node, parent);
    if (strcmp(type, "enum_item") == 0)               return handleEnum(ts_node, parent);
    if (strcmp(type, "trait_item") == 0)              return handleTrait(ts_node, parent);
    if (strcmp(type, "impl_item") == 0)               return handleImpl(ts_node, parent);
    if (strcmp(type, "union_item") == 0)              return handleStruct(ts_node, parent);
    ...
    if (strcmp(type, "let_declaration") == 0)         return handleLet(ts_node, parent);
    if (strcmp(type, "use_declaration") == 0)         return handleUse(ts_node, parent);
    if (strcmp(type, "mod_item") == 0)                return handleModule(ts_node, parent);
    if (strcmp(type, "macro_invocation") == 0)        return handleMacro(ts_node, parent);
    ...
    if (strcmp(type, "if_let_expression") == 0 ||
        strcmp(type, "while_let_expression") == 0) {
    ...
    if (strcmp(type, "expression_statement") == 0 ||
        ...
        strcmp(type, "block") == 0 ||
        strcmp(type, "declaration_list") == 0 ||
        strcmp(type, "token_tree") == 0 ||
        strcmp(type, "arguments") == 0 ||
        strcmp(type, "parameters") == 0 ||
        ...
        strcmp(type, "generic_function") == 0 ||
        strcmp(type, "where_clause") == 0 ||
        strcmp(type, "type_identifier") == 0 ||
        strcmp(type, "reference_type") == 0 ||
        strcmp(type, "pointer_type") == 0 ||
        strcmp(type, "array_type") == 0 ||
        strcmp(type, "tuple_type") == 0 ||
        strcmp(type, "trait_bound") == 0 ||
        strcmp(type, "for_lifetime") == 0 ||
        strcmp(type, "attribute_item") == 0 ||
        strcmp(type, "inner_attribute_item") == 0 ||
        strcmp(type, "visibility_modifier") == 0 ||
        strcmp(type, "mut_specifier") == 0 ||
        strcmp(type, "self_parameter") == 0 ||
        strcmp(type, "tuple_pattern") == 0 ||
        strcmp(type, "struct_pattern") == 0 ||
        strcmp(type, "reference_pattern") == 0 ||
        strcmp(type, "slice_pattern") == 0 ||
        strcmp(type, "or_pattern") == 0) {
...
Node* RustTranslator::handleMacro(TSNode ts_node, Node* parent) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (strcmp(t, "token_tree") == 0) continue;
        translateNode(child, parent);
    }
    return nullptr;
}
