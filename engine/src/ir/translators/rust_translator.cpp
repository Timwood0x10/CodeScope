...
Node *RustTranslator::translateNode(TSNode ts_node, Node *parent) {
  ...
  if (strcmp(type, "expression_statement") == 0 ||
      ...
      strcmp(type, "pointer_type") == 0 || strcmp(type, "array_type") == 0 ||
      strcmp(type, "tuple_type") == 0 || strcmp(type, "trait_bound") == 0 ||
...
Node *RustTranslator::handleImpl(TSNode ts_node, Node *parent) {
  ...
  Node *impl_class =
      impl_type ? resolveSymbol(std::string(impl_type)) : nullptr;
...
Translator *createRustTranslator() { return new RustTranslator(); }
