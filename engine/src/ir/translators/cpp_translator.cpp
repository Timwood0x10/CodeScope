...
TranslationUnit* CppTranslator::translate(TSTree* tree, const char* source,
...
std::string CppTranslator::extractName(TSNode ts_node) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (strcmp(t, "identifier") == 0) return nodeText(child);
        if (strcmp(t, "field_identifier") == 0) return nodeText(child);
        if (strcmp(t, "type_identifier") == 0) return nodeText(child);
        if (strcmp(t, "nested_identifier") == 0) return nodeText(child);
        if (strcmp(t, "qualified_identifier") == 0) return extractName(child);
...
Node* CppTranslator::translateNode(TSNode ts_node, Node* parent) {
    ...
    if (strcmp(type, "function_definition") == 0)  return handleFuncDef(ts_node, parent);
    if (strcmp(type, "declaration") == 0)           return handleDeclaration(ts_node, parent);
    if (strcmp(type, "class_specifier") == 0)       return handleClassSpec(ts_node, parent);
    if (strcmp(type, "struct_specifier") == 0)      return handleClassSpec(ts_node, parent);
    if (strcmp(type, "union_specifier") == 0)       return handleClassSpec(ts_node, parent);
    if (strcmp(type, "namespace_definition") == 0)  return handleNamespace(ts_node, parent);
    if (strcmp(type, "template_declaration") == 0)  return handleTemplate(ts_node, parent);
    if (strcmp(type, "enum_specifier") == 0)        return handleEnum(ts_node, parent);
    ...
    if (strcmp(type, "preproc_include") == 0)       return handleInclude(ts_node, parent);
    if (strcmp(type, "type_definition") == 0)       return handleTypeDef(ts_node, parent);
    if (strcmp(type, "parameter_declaration") == 0) return handleParamDecl(ts_node, parent);
    if (strcmp(type, "field_declaration") == 0)     return handleFieldDecl(ts_node, parent);
    ...
    if (strcmp(type, "cast_expression") == 0 ||
        strcmp(type, "static_cast_expression") == 0 ||
    ...
    if (strcmp(type, "expression_statement") == 0 ||
        ...
        strcmp(type, "base_class_clause") == 0 ||
        strcmp(type, "access_specifier") == 0 ||
        strcmp(type, "template_argument") == 0 ||
        strcmp(type, "template_parameter") == 0 ||
        strcmp(type, "optional_parameter_declaration") == 0 ||
        strcmp(type, "virtual_specifier") == 0 ||
        strcmp(type, "override_specifier") == 0 ||
        strcmp(type, "noexcept") == 0 ||
        strcmp(type, "lambda_expression") == 0 ||
        strcmp(type, "lambda_capture_specifier") == 0 ||
        strcmp(type, "enumerator_list") == 0 ||
        strcmp(type, "decltype") == 0 ||
...
Node* CppTranslator::handleClassSpec(TSNode ts_node, Node* parent) {
    ...
    if (!cls->name.empty()) defineSymbol(cls->name, cls);
...
Node* CppTranslator::handleEnum(TSNode ts_node, Node* parent) {
    ...
    if (!en->name.empty()) defineSymbol(en->name, en);
...
Node* CppTranslator::handleTypeDef(TSNode ts_node, Node* parent) {
    ...
    if (!alias->name.empty()) defineSymbol(alias->name, alias);
...
Node* CppTranslator::handleParamDecl(TSNode ts_node, Node* parent) {
    ...
    if (!param->name.empty()) defineSymbol(param->name, param);
