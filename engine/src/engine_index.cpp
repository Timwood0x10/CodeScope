			   ...
			   const char *language_filter)
{
	 ...
	 if (verbose)
	  fprintf(stderr, "BATCH [%zu..%zu] of %zu (%zu files)\n",
	   batch_start, batch_end - 1, jobs.size(), batch_count);
		...
		std::vector<std::unique_ptr<ir::TranslationUnit> > all_units(batch_count);
		std::vector<std::unique_ptr<ir::SemanticUnit> > semantic_units(batch_count);
		std::mutex collect_lock;
		std::atomic<int> next_job{0};

		auto translate_batch_worker = [&]() {
		 thread_local static std::unordered_map<std::string, TSParser *> tl_parsers;
		 thread_local static std::unordered_map<std::string, std::unique_ptr<ir::JsVisitor>> tl_visitors;

		 while (true) {
		  int local_idx = next_job.fetch_add(1);
		  if (local_idx >= static_cast<int>(batch_count)) break;
		  size_t global_idx = batch_start + local_idx;
		  auto &job = jobs[global_idx];
		  auto lit = lang_ptrs.find(job.lang);
		  if (lit == lang_ptrs.end()) continue;
		  const TSLanguage *ts_lang = lit->second;

		  // File size check — use pre-parsed max_file_size
		  struct stat file_stat;
		  if (stat(job.path.c_str(), &file_stat) == 0 &&
		      static_cast<uint64_t>(file_stat.st_size) > max_file_size) continue;
		  ...
		  if (vl == tl_visitors.end()) {
		   ...
		   if (v) { tl_visitors[job.lang] = std::unique_ptr<ir::JsVisitor>(v); visitor = v; }
		  } else { visitor = vl->second.get(); visitor->reset(); }

		  if (visitor) {
		   ir::SemanticUnit *su = visitor->visit(tree, source.c_str(), job.path.c_str());
		   ts_tree_delete(tree);
		   if (su) { std::lock_guard<std::mutex> lk(collect_lock); semantic_units[local_idx].reset(su); }
		   continue;
		  }

		  // Old pipeline fallback
		  auto translator = std::unique_ptr<ir::Translator>(ir::createTranslator(job.lang.c_str()));
		  if (!translator) { ts_tree_delete(tree); continue; }
		  ir::TranslationUnit *unit = translator->translate(tree, source.c_str(), job.path.c_str());
		  ts_tree_delete(tree);
		  if (unit) { std::lock_guard<std::mutex> lk(collect_lock); all_units[local_idx].reset(unit); }
		 }
		};

		int num_workers = std::min(static_cast<int>(batch_count),
		...
		for (int i = 0; i < num_workers; i++) {
		 ...
		 struct WA { decltype(translate_batch_worker) * fn; };
		 auto *a = new WA{&translate_batch_worker};
		 pthread_create(&workers[i], &attr, [](void *v) -> void * {
		  auto *w = static_cast<WA*>(v); (*w->fn)(); delete w; return nullptr; }, a);
		 pthread_attr_destroy(&attr);
		}
		for (auto &t : workers) pthread_join(t, nullptr);
		int64_t batch_parse_ms = duration_cast<milliseconds>(
		 steady_clock::now() - t_parse_start).count();
		...
		time_sqlite_ms += duration_cast<milliseconds>(
		 steady_clock::now() - t_sqlite_start).count();
	...
	time_buildgraph_ms = duration_cast<milliseconds>(
		steady_clock::now() - t_bg_start).count();
std::ostringstream result;
