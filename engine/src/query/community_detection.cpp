...
runLabelPropagation(const std::unordered_map<uint64_t, std::vector<uint64_t>> &adjacency,
...
std::string detectCommunities(uint64_t project_id, store::GraphStore *store) {
    ...
    std::sort(
        sorted_communities.begin(), sorted_communities.end(),
        [](const Community &a, const Community &b) { return a.member_count > b.member_count; });
