...
static bool
loadGraph(sqlite3 *db, uint64_t project_id,
	  ...
	  std::unordered_map<uint64_t, std::string> &node_files)
{
	// Load all graph nodes in this project
	{
		...
		const char *sql =
			"SELECT id, name, node_type, file_path FROM graph_nodes "
			"WHERE project_id = ?";
	...
	// Load all edges (both Calls and References) in this project
	{
		...
		while (sqlite3_step(stmt) == SQLITE_ROW) {
			...
			if (adjacency.count(src) && adjacency.count(tgt)) {
				...
				adjacency[tgt].push_back(
					src); // undirected for community detection
			}
		}
		sqlite3_finalize(stmt);
	}
	...
	int max_iterations = 20)
{
	...
	for (int iter = 0; iter < max_iterations; iter++) {
		...
		// Early exit if we did a full pass with no changes (check after first node
		// checks)
		if (!changed)
			break;
	}
...
static std::unordered_map<uint64_t, Community>
buildCommunities(const std::unordered_map<uint64_t, uint64_t> &assignments,
			      ...
			      bool include_members)
{
	...
	if (!loadGraph(db, project_id, adjacency, node_names, node_types,
		       node_files)) {
	...
	auto communities = buildCommunities(assignments, node_names, node_types,
					    node_files);
	...
	for (auto &[id, comm] : communities)
		sorted_communities.push_back(std::move(comm));
