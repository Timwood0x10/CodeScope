...
class QueryEngine {
  public:
    ...
    std::string getNeighbors(uint64_t project_id, uint64_t node_id, int edge_type_filter,
                             int radius);

    std::string findShortestPath(uint64_t project_id, uint64_t source_id, uint64_t target_id);

    std::string getSubgraph(uint64_t project_id, uint64_t center_node_id, int radius,
                            const char *node_type_filter, const char *edge_type_filter);

    std::string locateNode(uint64_t project_id, uint64_t node_id, int context_lines);
     ...
     */
    std::string traceCallChain(uint64_t project_id, const char *from_function,
