...
static const std::unordered_map<std::string, int> &typeMap() {
    static const std::unordered_map<std::string, int> m = {
        {"Function", 0}, {"Method", 1},   {"Class", 2},   {"Struct", 3},     {"Interface", 4},
        {"Variable", 5}, {"Module", 6},   {"File", 7},    {"References", 0}, {"Calls", 1},
        {"Defines", 2},  {"Contains", 3}, {"Imports", 4}, {"Inherits", 5},
    };
    return m;
}
...
static void parseNodeSpec(const std::string &spec, std::string &out_type, std::string &out_name) {
    out_type.clear();
    out_name.clear();
...
static std::string buildSingleHopQuery(uint64_t project_id, int edge_type, int src_type_val,
                                       int tgt_type_val, const std::string &src_name,
...
static std::string buildMultiHopQuery(uint64_t project_id, int edge_type, int min_depth,
                                      int max_depth, int src_type_val, int tgt_type_val,
    ...
    sql << "WITH RECURSIVE path(src_id, tgt_id, depth, chain) AS ("
        ...
        << "FROM path p "
        << "JOIN graph_edges ge ON ge.source_node_id = p.tgt_id "
        << "WHERE ge.project_id = " << project_id << " AND ge.edge_type = " << edge_type
        ...
        << "JOIN graph_nodes src ON src.id = p.src_id "
        << "JOIN graph_nodes tgt ON tgt.id = p.tgt_id "
        << "WHERE p.depth >= " << min_depth << " AND p.depth <= " << max_depth;
...
std::string executeGraphQuery(uint64_t project_id, const char *dsl_query,
    ...
    if (multi_hop) {
        ...
        sql = buildMultiHopQuery(project_id, edge_type, min_depth, max_depth, src_type_val,
                                 tgt_type_val, src_name, tgt_name);
    } else {
        sql = buildSingleHopQuery(project_id, edge_type, src_type_val, tgt_type_val, src_name,
                                  tgt_name);
    }
