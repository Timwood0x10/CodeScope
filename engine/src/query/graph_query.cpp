...
static std::string buildSingleHopQuery(uint64_t project_id,
    int edge_type, int src_type_val, int tgt_type_val,
    const std::string& src_name, const std::string& tgt_name)
{
    ...
    if (edge_type >= 0) sql << " AND ge.edge_type = " << edge_type;
    if (src_type_val >= 0) sql << " AND src.node_type = " << src_type_val;
    if (tgt_type_val >= 0) sql << " AND tgt.node_type = " << tgt_type_val;
    if (!src_name.empty()) sql << " AND src.name = '" << src_name << "'";
    if (!tgt_name.empty()) sql << " AND tgt.name = '" << tgt_name << "'";
    sql << " LIMIT 200";
    return sql.str();
}
...
static std::string buildMultiHopQuery(uint64_t project_id,
    int edge_type, int min_depth, int max_depth,
    ...
    const std::string& src_name, const std::string& tgt_name)
{
    ...
    sql << "WITH RECURSIVE path(src_id, tgt_id, depth, chain) AS ("
        ...
        << ") "
        << "SELECT src.id AS src_id, src.name AS src_name, src.node_type AS src_type, "
        << "src.file_path AS src_file, "
        << "tgt.id AS tgt_id, tgt.name AS tgt_name, tgt.node_type AS tgt_type, "
        << "tgt.file_path AS tgt_file, "
        << "p.depth, p.chain "
...
std::string executeGraphQuery(uint64_t project_id, const char* dsl_query,
    ...
    if (star_pos != std::string::npos) {
        ...
        if (dotdot != std::string::npos) {
            ...
            trim(min_s); trim(max_s);
            if (!min_s.empty()) min_depth = std::atoi(min_s.c_str());
            if (!max_s.empty()) max_depth = std::atoi(max_s.c_str());
    ...
    if (multi_hop) {
        if (edge_type < 0) {
            return "{\"total\":0,\"results\":[],\"error\":\"multi-hop queries require an edge type\"}";
        }
        sql = buildMultiHopQuery(project_id, edge_type, min_depth, max_depth,
                                 src_type_val, tgt_type_val, src_name, tgt_name);
    } else {
        sql = buildSingleHopQuery(project_id, edge_type,
                                  src_type_val, tgt_type_val, src_name, tgt_name);
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (multi_hop) {
            ...
            json << "\"target\":{"
                 ...
                 << "\"name\":\"" << (sqlite3_column_text(stmt, 5) ? reinterpret_cast<const char*>(sqlite3_column_text(stmt, 5)) : "") << "\","
                 << "\"type\":" << sqlite3_column_int(stmt, 6) << ","
                 << "\"file\":\"" << (sqlite3_column_text(stmt, 7) ? reinterpret_cast<const char*>(sqlite3_column_text(stmt, 7)) : "") << "\""
                 << "},"
                 << "\"depth\":" << sqlite3_column_int(stmt, 8) << ","
                 << "\"chain\":\"" << (sqlite3_column_text(stmt, 9) ? reinterpret_cast<const char*>(sqlite3_column_text(stmt, 9)) : "") << "\"";
        } else {
            json << "\"edge\":{"
                 << "\"id\":" << sqlite3_column_int64(stmt, 4) << ","
