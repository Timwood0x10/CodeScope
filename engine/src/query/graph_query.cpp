...
static const std::unordered_map<std::string, int> &typeMap() {
  static const std::unordered_map<std::string, int> m = {
      ...
      {"Interface", 4},  {"Variable", 5}, {"Module", 6},  {"File", 7},
      {"References", 0}, {"Calls", 1},    {"Defines", 2}, {"Contains", 3},
...
static std::string buildMultiHopQuery(uint64_t project_id, int edge_type,
  ...
  sql << "WITH RECURSIVE path(src_id, tgt_id, depth, chain) AS ("
      ...
      << " AND ge.edge_type = " << edge_type << " AND p.depth < " << max_depth
      << ") "
...
std::string executeGraphQuery(uint64_t project_id, const char *dsl_query,
  ...
  if (multi_hop) {
    ...
    sql = buildSingleHopQuery(project_id, edge_type, src_type_val, tgt_type_val,
                              src_name, tgt_name);
  }
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    if (multi_hop) {
      ...
      json << "\"edge\":{"
           ...
           << "\"name\":\""
           << (sqlite3_column_text(stmt, 7) ? reinterpret_cast<const char *>(
                                                  sqlite3_column_text(stmt, 7))
                                            : "")
           << "\","
           << "\"type\":" << sqlite3_column_int(stmt, 8) << ","
           << "\"file\":\""
           << (sqlite3_column_text(stmt, 9) ? reinterpret_cast<const char *>(
                                                  sqlite3_column_text(stmt, 9))
                                            : "")
           << "\""
           << "}";
