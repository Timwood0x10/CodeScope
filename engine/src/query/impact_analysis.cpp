...
static std::vector<std::string> parseFileList(const char* json) {
    ...
    if (!json || !*json) return files;

    const char* p = json;
    // Skip whitespace
    while (*p == ' ' || *p == '\t' || *p == '\n' || *p == '\r') p++;
    if (*p != '[') return files; // not an array
    p++;
    ...
    while (*p) {
        ...
        while (*p == ' ' || *p == '\t' || *p == '\n' || *p == '\r' || *p == ',') p++;
        if (*p == ']') break;
        if (*p != '"') return files; // expected string
        ...
        if (*p != '"') return files; // unterminated string
    ...
    std::ostringstream& json, bool& first)
{
    ...
    sqlite3_stmt* stmt = nullptr;
    std::string sql =
        "SELECT DISTINCT src.id, src.name, src.file_path, "
        ...
        "AND ge.target_node_id IN (" + id_list + ") "
        "LIMIT 100";
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        json << "{"
             ...
             << "\"file\":\"" << (sqlite3_column_text(stmt, 2) ? reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2)) : "") << "\","
             << "\"caller_of\":\"" << (sqlite3_column_text(stmt, 3) ? reinterpret_cast<const char*>(sqlite3_column_text(stmt, 3)) : "") << "\""
             << "}";
    }
    sqlite3_finalize(stmt);
}
    ...
    std::ostringstream& json, bool& first)
{
    ...
    sqlite3_stmt* stmt = nullptr;
    std::string sql =
        "SELECT DISTINCT tgt.id, tgt.name, tgt.file_path, "
        "src.name AS callee_of "
        "FROM graph_edges ge "
        "JOIN graph_nodes src ON src.id = ge.source_node_id "
        "JOIN graph_nodes tgt ON tgt.id = ge.target_node_id "
        "WHERE ge.project_id = ? AND ge.edge_type = 1 "
        "AND ge.source_node_id IN (" + id_list + ") "
        "LIMIT 100";
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        json << "{"
             ...
             << "\"name\":\"" << (sqlite3_column_text(stmt, 1) ? reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1)) : "") << "\","
             << "\"file\":\"" << (sqlite3_column_text(stmt, 2) ? reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2)) : "") << "\","
             << "\"callee_of\":\"" << (sqlite3_column_text(stmt, 3) ? reinterpret_cast<const char*>(sqlite3_column_text(stmt, 3)) : "") << "\""
             << "}";
    }
    sqlite3_finalize(stmt);
}
    ...
    const char* modified_files_json)
{
    ...
    if (!modified_ids.empty()) {
        ...
        sqlite3_bind_int64(stmt, 1, static_cast<int64_t>(project_id));
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            all_impacted.insert(static_cast<uint64_t>(sqlite3_column_int64(stmt, 0)));
        }
        sqlite3_finalize(stmt);
