...
std::string QueryEngine::getModuleMap(uint64_t project_id) {
    ...
    for (const auto& dir : dirs) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            first_fn = false;
            json << "{"
                 << "\"name\":\"" << (sqlite3_column_text(stmt, 0) ? (const char*)sqlite3_column_text(stmt, 0) : "") << "\","
                 << "\"type\":" << sqlite3_column_int(stmt, 1) << ","
                 << "\"file\":\"" << (sqlite3_column_text(stmt, 2) ? (const char*)sqlite3_column_text(stmt, 2) : "") << "\","
...
std::string QueryEngine::getEntryPoints(uint64_t project_id) {
    ...
    std::string sql = "SELECT gn.id, gn.name, gn.node_type, gn.file_path, "
                       ...
                       "LEFT JOIN node_complexity nc ON nc.graph_node_id = gn.id "
                       "WHERE gn.project_id = ? AND gn.node_type IN (0,1) "
                       "AND gn.name IN ('main','Main','run','Run','start','Start','init','Init','setup','Setup') "
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            json << "{"
                 ...
                 << "\"file\":\"" << (sqlite3_column_text(stmt, 3) ? (const char*)sqlite3_column_text(stmt, 3) : "") << "\","
...
std::string QueryEngine::traceCallChain(uint64_t project_id,
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        json << "{\"found\":true,"
             << "\"chain\":\"" << (sqlite3_column_text(stmt, 0) ? (const char*)sqlite3_column_text(stmt, 0) : "") << "\","
...
std::string QueryEngine::getProjectOverview(uint64_t project_id) {
    ...
    {
        sqlite3_stmt* stmt = nullptr;
        sqlite3_prepare_v2(db, "SELECT COUNT(*) FROM graph_nodes WHERE project_id=?", -1, &stmt, nullptr);
        sqlite3_bind_int64(stmt, 1, static_cast<int64_t>(project_id));
        if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        sqlite3_prepare_v2(db, "SELECT COUNT(*) FROM graph_edges WHERE project_id=?", -1, &stmt, nullptr);
        sqlite3_bind_int64(stmt, 1, static_cast<int64_t>(project_id));
        if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        json << "\"languages\":[";
        sqlite3_prepare_v2(db, "SELECT language,COUNT(*) FROM files WHERE project_id=? GROUP BY language", -1, &stmt, nullptr);
        sqlite3_bind_int64(stmt, 1, static_cast<int64_t>(project_id));
        bool first = true;
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            if (!first) json << ",";
            first = false;
            json << "{\"lang\":\"" << (sqlite3_column_text(stmt, 0) ? (const char*)sqlite3_column_text(stmt, 0) : "") << "\","
