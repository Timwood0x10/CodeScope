...
char *engine_index_file(uint64_t project_id, const char *file_path) {
        ...
        for (auto *node : unit->all_nodes) {
          ...
          if (ext_cache.count(node->name)) {
            ...
            std::string def = lsp.queryDefinition(
                file_path, static_cast<int>(node->loc.start_row),
  ...
  for (auto *node : unit->all_nodes) {
    ...
    uint64_t db_id = g_store->insertIRNode(
        ...
        node->loc.start_row, node->loc.start_col, node->loc.end_row,
    ...
    const char *fts_qn =
        node->qualified_name.empty() ? nullptr : node->qualified_name.c_str();
    const char *fts_comment =
        node->doc_comment.empty() ? nullptr : node->doc_comment.c_str();
    if (fts_name || fts_qn || fts_comment) {
      g_store->insertIntoFTS(db_id, project_id, fts_name, fts_qn, file_path,
                             fts_comment, static_cast<int>(node->kind));
    }
  ...
  for (auto *node : unit->all_nodes) {
    for (auto &edge : node->semantic_edges) {
      ...
      if (src_it != ir_id_to_db_id.end() && tgt_it != ir_id_to_db_id.end()) {
        g_store->insertIRSemanticEdge(project_id, src_it->second,
                                      tgt_it->second,
      ...
      if (gn.type == graph::NodeType::Function ||
        ...
        for (auto *ir_node : unit->all_nodes) {
          if (ir_node->id == gn.ir_node_id) {
            ...
            g_store->setComplexity(project_id, gn.id, cr.cyclomatic,
                                   cr.cognitive, cr.nesting_depth,
                                   cr.decision_points);
            break;
          }
        }
...
static std::string extractName(std::string_view line, const std::string &kind,
  ...
  if (lang == "rust") {
    ...
    static const char *rkeywords[] = {
        "pub unsafe fn", "pub async fn", "pub fn",    "fn",        "pub struct",
        "struct",        "pub enum",     "enum",      "pub trait", "trait",
        "pub type",      "type",         "pub const", "const",     "pub static",
        "static",        "pub union",    "union",     "mod",       nullptr};
    ...
    if (paren != std::string_view::npos && paren > 0) {
      auto name_end = paren;
      while (name_end > 0 &&
             (isalnum(line[name_end - 1]) || line[name_end - 1] == '_'))
        name_end--;
      if (name_end < paren) {
        auto name_start = name_end;
        // Skip C++ namespace prefix T::
        if (name_start >= 2 && line[name_start - 1] == ':' &&
            line[name_start - 2] == ':')
          name_start -= 2;
...
static bool isEntryPoint(const std::string &name) {
  static const char *ep_names[] = {
      "main", "Main",  "init",  "Init",    "setup",   "Setup", "run",
...
char *engine_scan_project(uint64_t project_id, const char *dir_path,
      ...
      if (it->is_regular_file()) {
        ...
        if (mod_it != module_path_map.end()) {
          ...
          for (const auto &part : p) {
            ...
            uint64_t mod_id = g_store->insertModule(project_id, current_parent,
                                                    part.string().c_str(),
                                                    accumulated.c_str(), "");
        ...
        while (std::getline(stream, line)) {
          ...
          int span_start = static_cast<int>(
              content.data() - content.c_str()); // not right, approximate
          ...
          uint64_t sym_id = g_store->insertSymbol(
              project_id, parent_mod_id, kind.c_str(), name.c_str(),
              sig.c_str(), visibility.c_str(), lang, file_path.c_str(),
              line_num, 1, 0, 0);
  ...
  std::unordered_map<uint64_t, int> mod_file_counts;
  {
    ...
    if (sqlite3_prepare_v2(db, count_sql.c_str(), -1, &stmt, nullptr) ==
        SQLITE_OK) {
      ...
      while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (sqlite3_prepare_v2(db, cnt_sql, -1, &cnt_stmt, nullptr) ==
            SQLITE_OK) {
          ...
          if (sqlite3_step(cnt_stmt) == SQLITE_ROW) {
            ...
            if (sqlite3_prepare_v2(db, upd_sql, -1, &upd_stmt, nullptr) ==
                SQLITE_OK) {
...
static uint64_t lookupSymbolId(
    uint64_t project_id, const std::string &file_path, const std::string &name,
    int line,
    std::unordered_map<std::string,
                       std::vector<std::pair<std::string, uint64_t>>> &cache) {
...
char *engine_enhance_project(uint64_t project_id) {
  ...
  for (const auto &file_path : files) {
    ...
    ir::TranslationUnit *unit =
        translator->translate(tree, source.c_str(), file_path.c_str());
    ts_tree_delete(tree);
    if (!unit)
      continue;
    ...
    // ─────────────────────────────────────────────────────
    {
      ...
      for (auto *node : unit->all_nodes) {
        ...
        if (node->kind == ir::NodeKind::FunctionDecl ||
          ...
          int lines =
              static_cast<int>(node->loc.end_row - node->loc.start_row + 1);
          ...
          g_store->insertMetric(
              ...
              static_cast<int>(cr.cognitive), lines, param_count, call_count,
          ...
          g_store->insertEmbedding(sym_id, vec.data(),
          ...
          int lines =
              static_cast<int>(node->loc.end_row - node->loc.start_row + 1);
          g_store->insertMetric(project_id, "symbol", sym_id, 0, 0, 0, lines, 0,
                                0, 0, 0);
...
char *engine_get_neighbors(uint64_t project_id, uint64_t node_id,
  ...
  return dupString(
      g_query->getNeighbors(project_id, node_id, edge_type_filter, radius));
}
...
char *engine_search_semantic(uint64_t project_id, const char *query,
                             int limit) {
  if (!g_store)
    return dupString(
        "{\"total\":0,\"results\":[],\"error\":\"not initialized\"}");
  ...
  auto blob = vector_search::serializeVector(vec);
  return dupString(
      g_store->searchSemantic(project_id, blob.data(), blob.size(), limit));
}
...
void engine_free_string(char *ptr) { free(ptr); }
  ...
  for (auto &b : batches) {
    ...
    for (auto *node : b.unit->all_nodes) {
      uint64_t db_id = g_store->insertIRNode(
          ...
          node->name.empty() ? nullptr : node->name.c_str(), nullptr,
          node->loc.start_row, node->loc.start_col, node->loc.end_row,
          node->loc.end_col, node->language.c_str());
      ir_map[node->id] = db_id;
      const char *fn = node->name.empty() ? nullptr : node->name.c_str();
      if (fn || !node->doc_comment.empty())
        g_store->insertIntoFTS(db_id, project_id, fn, nullptr,
                               b.file_path.c_str(), node->doc_comment.c_str(),
...
static const char *detectLicense(const std::string &content) {
  ...
  if (content.find("GNU GENERAL PUBLIC LICENSE") != std::string::npos)
    return content.find("Version 3") != std::string::npos ? "GPL-3.0"
                                                          : "GPL-2.0";
...
char *engine_get_project_info(uint64_t project_id) {
  ...
  {
    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(db, "SELECT COUNT(*) FROM files WHERE project_id=?",
