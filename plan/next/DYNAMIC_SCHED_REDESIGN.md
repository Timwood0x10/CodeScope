...
> **Status**: Implemented — the chunk-level shared queue, work-stealing and static
> CPU policy specified here are what `server/src/scheduler/chunked.rs` runs, and
> those files cite this document by section number.
> **Author**: 调度层改造设计
> **Date**: 2026-07-19
> **Scope**: 取代方案 E 的实现现状（模块级分配 + CPU 核心动态借用），改为 chunk 级共享队列 + work-stealing + 静态 CPU。
> **History**: 本文所取代的方案 E（`builtin-scheduler-design.md`）与它的关联审查
> （`CODE_REVIEW_DYNAMIC_SCHED_2026-07-19.md`，DS-1~DS-6）在重设计落地后已从仓库移除，
> 下文对它们的引用只作为历史保留。
...
当前实现（方案 E 的落地）有两个**结构性**病灶，靠打补丁修不掉：
...
1. **扫模块**：对单个模块目录跑 `FilterPolicy::shouldSkipEntry`（真相源），得到 candidate 文件全列表（路径 + 字节数）。
