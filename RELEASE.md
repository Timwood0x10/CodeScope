...
Speeds up full (non-fast) indexing and fixes a resolver JOIN defect. Dead `Literal`/`Variable` rows are no longer written to `semantic_records` (the full-index `buildGraph` and all queries ignore them; only the in-memory GraphBuilder uses Variable nodes, which stay intact), shrinking the DB write and index rebuild hot spots — measured on CodeScope's own engine: 95,944 → 25,146 rows, full-index time 2.18s → ~1.36s (**-38%**). The `global_var_types_`/`global_struct_fields_` preloads previously joined `semantic_records` on `original_id` without a `file_path` term; since `original_id` is per-file, this cross-matched several files' same-id parents and inflated the join ~35x. Adding `file_path` cut resolver 11.2s → 5.4s and the goagent full index 20.6s → 14.6s (**-29%**) while resolving more refs correctly (accuracy gate stays P/R/F1 = 1.0).
