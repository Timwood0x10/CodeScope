...
- **Community detection restored and exposed as the `get_communities` MCP tool** (`query/query_communities.cpp`, `engine_ffi.cpp`, `server/src/ffi/mod.rs`, `server/src/tools/{mod.rs,catalog.rs}`): the Phase-0 cut removed the label-propagation algorithm and left `QueryEngine::getCommunities` as a stub returning `{"communities":[],"total":0}`, while the C ABI export stayed in `engine.h`'s "NOT bound by the server" list — so external docs said "the engine has it, MCP doesn't" while the implementation was in fact gone everywhere. The algorithm is reimplemented over the canonical `relation` table (type=1 CALLS, treated as undirected; self-loops carry no signal and isolated entities have no community, so both are excluded) using **synchronous (Jacobi) label propagation**: a sequential sweep lets a single label cross an entire connected component in one pass, which collapses two dense triangles joined by one bridge edge into one giant community — verified case in `test_communities`. Nodes are visited in ascending id order and ties prefer the node's current label, then the smallest label id, so the same database always yields the same communities. Output is summary-first — `{id, label, member_count}` per community, with members only when `include_members` is set — and `max_communities` (default 20, cap 500) / `max_members` (default 10, cap 200) are clamped engine-side and reported through `truncated`. An earlier revision of this feature emitted every member of every community: ~342 KB / ~100K tokens for a single query, the opposite of what this project is for. The default response returns the largest communities (on the self index: 78/78/48/48/19 members, 233 communities over 1,370 call edges).
- **`test_communities`** (`engine/tests/test_communities.cpp`): two triangles joined by a single bridge must remain two three-member communities (this is what pins the synchronous-update decision), the representative must be each community's highest-degree member, and self-loops, isolated entities, References(0) edges, byte-level determinism, the summary-first default, community/member clamping (including the `truncated` flag) and the empty-graph case are all asserted.

### 🐛 Bug Fixes

- **`get_verifier_registry_status` now honours its documented `project_id` argument** (`server/src/tools/mod.rs`): the handler opened with `let _ = args;`, so the argument the schema documents — including "pass project_id=0 to skip the evidence backend probe" — was discarded and the server's own project always used. The argument now wins, with the server's project as the fallback for clients that send none.
- **`codescope_trace`'s documented depth limit matches the enforced one** (`server/src/tools/catalog.rs`): the schema said "max: 5" while `MAX_TRAVERSAL_DEPTH` is 10 and `clamp_depth` allows exactly that, so the tool description understated what the server accepts. The schema now states 10 and names the constant.
- **Removed an unreachable `is_source_file` entry** (`server/src/discover.rs`, `server/src/tools/discover.rs`): `is_source_file` derives the extension from the LAST dot, so `"foo.d.ts"` already yields `".ts"` and the `".d.ts"` allow-list pattern could never match anything. Both copies are gone, replaced by a comment explaining why they must not return, and the existing extension test now asserts `is_source_file("types.d.ts")` is still true — the behaviour is unchanged, the entry was pure noise.
- **Dead code removed (1.5k lines that could be edited by mistake)**: `ir/translators/scanner_visitor.{cpp,h}` (447 + 63 lines, no caller in `engine/`, `include/` or `server/` — deleting it also removes the only four `isalnum`/`tolower` raw-`char` UB sites), the superseded resolver stack `resolver.{cpp,h}` / `project_resolver.{cpp,h}` / `project_index.{cpp,h}` / `resolve_cache.{cpp,h}` (643 lines, dropped from the CMake source list as well), `SemanticEmitter::emitScope`, `CTranslator::handleAttributedDeclarator`, the five zero-call-site scorers (`factorImportMatch`, `factorNamespaceMatch`, `factorDistanceMatch`, `factorReceiverMatch`, `factorReceiverTypeMatch`), `computeTotalScore`, `ScoredCandidate`, `anyImportMatches`, `moduleTokenFromPath`, and `go_visitor.cpp`'s `method_spec` branch — unreachable, since the linked grammar (`tree-sitter-go.so`) exposes `method_elem` and not `method_spec`. Nine comments that told readers to keep the inline scoring "in sync with factorX in factors.cpp" were rewritten: there is only one copy now. One live function, `buildReceiverMatchContext`, sat inside a deleted range and was caught by the linker rather than by review; it is restored, and every deleted file was re-verified per symbol (`make check`: 81 engine tests, TP 36 / FP 0 / FN 0 — identical to before, which is the evidence the removed code was dead).
- **Rust no longer drops calls to builtin-named functions the file defines itself** (`ir/translators/rust_visitor.cpp`): the other four language filters were given a "a name this file declares is user code" exemption, but the Rust one was missed — `isRustBuiltin(qualified)` ran on its own, so a Rust file defining `fn write()` and calling `write(1)` lost the call record entirely (a silent false negative: the resolver can only emit edges for records that exist). It now consults the same exemption (`isLocallyDefined`), while the filter keeps matching the qualified text so `Type::new` is still not filtered.
- **The builtin-name exemption is now covered by tests** (`tests/test_builtin_method_calls.cpp`): it was the one part of the exemption fix with no test at all. The suite gains one locally-defined collision per language (`py` `format`, `js` `Map`, `java` `format`, `c` `free`, `rs` `write`), each paired with a negative control — a bare call to a builtin the file does NOT define, which must stay filtered, so the exemption can never be mistaken for "the filter is off". Falsified both ways before being trusted: forcing `isLocallyDefined` to return false drops `own.py`'s `format`, and disabling only the Rust exemption drops `own.rs`'s `write`.
- **The dispatch expansion no longer crosses languages** (`resolver/pipeline.cpp`): it selected targets by `qualified_name` prefix alone, without the language hard filter the other two resolution paths apply, so a `.cpp` call site on an interface implemented in both C++ and Python gained a CALLS edge to the Python implementation. Verified by falsification — short-circuiting the new filter makes `test_resolution_kind` emit `kind=dispatch impl=PyImpl` for a C++ call site and its assertion fail; with the filter the edge is gone.
- **A fuzzy candidate is no longer labelled `exact_local`** (`resolver/pipeline.cpp`): the single-candidate fast path runs whenever the candidate set holds one same-directory entity, and `cands` also points at the fuzzy fallback's vector — so a name that only matched by prefix (`"DeltaThi"` → `"DeltaThing"`) was recorded as an exact local hit, in the column the per-kind audits group by. The label now comes from a per-ref `used_fuzzy` flag (`fuzzy_local`). The same edit removed `from_fuzzy = (exact_hits == 0)`: `exact_hits` is a running total over ALL references, so it described nothing about the reference being resolved.
- **A filename coincidence no longer scores like receiver evidence** (`resolver/factors.cpp`): `factorReceiverTypeMatchPrecomp`'s weak fallback — a candidate whose FILE NAME contains the receiver type — returned `kScorePartialMatch`, the same value as a `qualified_name` that really contains it, so `Widgets.cpp` for receiver `Widgets` was indistinguishable from a genuine partial receiver match. It returns `kScoreWeakFilenameMatch` (0.2), preserving the ordering (partial 0.5 > coincidence 0.2 > none 0.0) while separating the two.
- **`resolution_kind` now names the evidence that decided the match** (`resolver/pipeline_apply.cpp`, `resolver/pipeline.cpp`): the column was read off "which reference field is non-empty" (`receiver_type > qualified_target > import_alias > name`), which answers a different question — a call carrying a `receiver_type` field but resolved by namespace/import evidence was still labelled `receiver_type`, and `qualified` was handed out the same way. Per-kind accuracy audits group by this column, so all of them inherited that skew. The hot path deliberately does not materialise a `FactorResult` vector per candidate (measured as a hot-loop win), so `applyConstraints` now records the largest positive factor contribution on the candidate while it accumulates the weighted sum, and the edge is labelled from that factor — with an exact qualified-name hit still checked explicitly, because it is not a scoring factor. Verified by a new `test_resolution_kind`: two fixtures whose evidence fields are present but non-decisive are now labelled `imported`/`decided_by=ImportMatch` where the old rule said `receiver_type` and `qualified`. The edges themselves are unchanged (accuracy gate still TP 36 / FP 0 / FN 0).
- **A single-file graph no longer carries a nameless ghost node** (`graph/graph_builder.cpp`): every visitor emits an empty-named `Variable` at `parent_id 0` as a container so the other records have a parent, and `buildSymbolGraph` meant to skip it — but the guard tested `recordKindToNodeType(kind) == NodeType::File`, which is never true for a Variable record, so the container became a nameless node. The bulk (entity) path never stores it, so the same file produced different graphs depending on how it was analysed. Covered by a new case in `test_graph_semantic`.
- **A partially built CSR is no longer published as a successful build** (`store/store_graph_csr.cpp`): adjacency groups whose INSERT was rejected were logged and then forgotten — the savepoint was released and the function returned true, so queries silently saw a graph with edges missing. The failures are counted now, and the build rolls back (leaving the previous CSR intact) and reports them.
- **A failed bulk COMMIT is no longer invisible** (`store/store_membulk.cpp`): `flush()` ignored `commitTransaction()`, so a failed COMMIT left the connection inside an open transaction — failing every later BEGIN — while reporting a bulk load that never landed. It now rolls back and returns false.
- **Parse-failure batches are transactional and honest** (`store/store_parse_failure.cpp`): the BEGIN and COMMIT results were ignored and a failed row was merely logged while the batch was still committed, so a partially written batch was indistinguishable from a complete one. All three are checked now; a partial batch is rolled back and reported.
- **Full-text search no longer returns "no results" for a query it never ran** (`store/store_query.cpp`): the FTS5 query was built from the raw input, so an unescaped `"` or `(` made the `MATCH` fail — and the failure fell through silently, so a quoted query looked like a query that matched nothing. The query is wrapped with `fts5Phrase` (the helper the schema comments already claimed was used here) and a step error is logged with `[module=store, method=searchUnifiedJson]` instead of vanishing.
- **The bulk-load index lists stay in step, and their comments tell the truth** (`store/store_membulk.cpp`): `idx_sr_oid` was in neither the drop list nor the create list, making the pair asymmetric, and the comments said "9 lookup indexes" while listing 10. Both lists now carry the same 11 indexes the schema creates.
- **`getSubgraph` / `getNeighbors` / the dead Cypher path** (`query/query_engine_traverse.cpp`, `query/graph_query.cpp`): the stale `(void)radius;` in `getSubgraph` contradicted the code three lines below it that honours radius; `getNeighbors` does not walk past one hop (the tool schema has always said "reserved, currently 1"), and its response now reports `radius_applied` — plus `radius_requested` and a note when a deeper walk was asked for — instead of silently answering a shallower question. `graph_query.cpp` had a `cypher` string that was assembled and never read; the three helpers that built it are deleted along with their comments about injection protection for a string no code path executed.
- **Go RWMutex acquisitions are no longer dropped from the facts** (`model/semantic_fact_extractor.cpp`): the guard that classified a lock call demanded a '.' immediately before "Lock", so `rw.RLock()` / `rw.WLock()` never entered the branch — they produced no sync fact at all — and the inner R/W checks could not be reached, which left the `rwmutex_usage` rule permanently empty. The RWMutex unlock forms were not even selected by the query. One classifier now handles `.Lock` / `.RLock` / `.WLock` and the matching Unlock forms, so those acquisitions are recorded (as `rwmutex`, rather than silently dropped) and a lock without its unlock is no longer reported just because `RUnlock` was invisible.
- **Capabilities are named after whole lines, and are no longer duplicated** (`model/plugins/capability.cpp`, `store/store_knowledge.cpp`): the README scan advanced on any '-' or '*' as well as a newline, so it restarted mid-line and named a capability after the fragment following the hyphen — "A thread-safe design - Supports fast lookup" became `FastLookup`, a name no code can match, which the capability-drift detector then reported as undocumented. Lines are now the unit, bullets still lose their marker, and `insertCapability` is guarded (plus a migration that collapses the copies earlier runs left behind), so re-indexing no longer multiplies capability rows and the drift counts derived from them.
- **A rule that found nothing no longer looks like an inspector that produced evidence** (`evidence/evidence_builder.cpp`): Count-mode rules emitted an `Evidence` with `count=0`, so a rule with zero matches still counted in `overall.inspectors_ran` and inflated the confidence derived from it. Every other combine mode returns no evidence when nothing matches, and Count now does the same.
- **`FunctionImplementsVerifier`'s comment now matches its code** (`verify/function_implements_verifier.cpp`): it claimed the verdict was "downgraded to `PartiallyVerified`" — a value the claim `Verdict` enum does not have — while the code reported `Supported` with a reduced confidence and a detail string saying the check was structural only. The constant is renamed `kConfFunctionStructuralOnly` and the comment states the real contract.
- **Builtin-name filtering no longer deletes calls to the file's own functions** (`ir/translators/*`): the filters keep pre-declared library symbols out of the graph but match on NAME only, so a user function whose name collides with a builtin (`def format()`, a locally declared `valueOf`, `import static …valueOf`, `free()`) had its call dropped entirely — a systematic false negative in the call graph. Each visitor now collects the names the file declares before visiting and keeps those calls, and imported names are kept too. Measured: the accuracy gate is unchanged at TP 36 / FP 0 / FN 0, while `def format()` + `format(1)` now yields a resolved call edge (`exact_local`) where it produced none.
- **The Aho-Corasick dispatch automaton is built thread-safely** (`ir/translators/js_visitor.cpp`, `ahocorasick.h`): its lazy init (`static bool built = false; if (!built) { …addPattern…build()… }`) was unsynchronised while parsing runs on `std::thread` workers, so two threads reaching their first JavaScript file together mutated the same automaton (`next[]`/`fail`/`out_link`) concurrently. It is now a function-local static object, initialised once under the C++11 guard variable; copying is deleted because the class owns raw nodes and its user-declared destructor suppressed the move constructor, so returning it by value would have silently double-freed.
- **LSP client: no unbounded write, no mistaking a notification for the response, no zombie** (`lsp/lsp_client.cpp`, `lsp/lsp_framing.h`): writes wait for writability with a deadline, so a server that stopped reading its stdin cannot hang the client forever; reads accumulate in a persistent buffer and only a response whose id matches answers the call, so a `window/logMessage` arriving first is skipped instead of returned as the definition result, and a message split across reads is reassembled; `stop()` waits for the child within a grace period, force-kills it, reaps it (no zombie) and reports a non-zero exit. Covered by `test_lsp_framing` (6 cases) — one of which caught a flaw in the first version of this fix, where a discarded junk frame was reported as "need more bytes" while a complete answer already sat in the buffer.
- **The merge refuses to proceed on module DBs that disagree about the schema** (`scheduler/merge_fetch.rs`, `scheduler/merge_driver.rs`): the merge generated its INSERT column lists from module 0 and applied them to every module, so a module DB left over from another build (or preserved by an incremental run) had its extra column silently default-filled — its data dropped with no message — or aborted the script mid-way with earlier modules already committed, leaving a partially merged `main.db` indistinguishable from a clean one. A consistency check now runs before anything is written (one `sqlite3` process per 8 modules, comparing `pragma_table_info(table, schema)` across the attached module DBs) and fails naming the module, table and column difference. The column fetch covers every table, not only the `skip_rowid`/`remap` ones.
- **Files directly in the target directory are indexed** (`discover.rs`, `scheduler/worker.rs`, `scheduler/quarantine.rs`): `discover_modules` only ever descended into subdirectories, so a flat project (all sources in one directory, or a leaf such as `<project>/src`) reported `"modules":[]` and indexed nothing at all, and a project mixing root files with subdirectories silently lost the root files (`top.go` + `sub/b.go` reported one file, not two). The root's own files are now a module (`discover::ROOT_MODULE_NAME`), whose worker is rooted at the project directory with every subdirectory excluded via `CODESCOPE_EXCLUDE_PATHS` (`dir/**` — the only form that prunes a directory *and* its contents), so the split stays exact and quarantine's retry keeps working for it. Verified end-to-end: the flat fixture went from `modules:[]` to 1 file / 24 nodes and the mixed one from 1 to 2 files / 48 nodes, with no double counting.
- **A run that discovered no source files is no longer reported as a success** (`scheduler/mod.rs`, `scheduler/dynamic.rs`): the early return emitted `{"ok":true,"modules":[],"note":"no source modules found"}` with no `complete` key, so an empty index was indistinguishable from a successful one — and that is what hid the discovery defect above. It now reports `ok:false, complete:false`.
- **One file, one identity: the single-file entry points no longer store a second spelling of an already-indexed file** (`engine_index_paths.h`, `engine_index.cpp`, `engine_index_files.cpp`): the walk stores `filesystem::path(root_argument) / entry`, so a `.`-rooted project holds `./src/a.go` and an absolute-rooted one holds `/abs/proj/src/a.go`, while `index_file` / `force_index_files` / the scheduler's `--file-list` retry were handed absolute paths by their callers and stored them verbatim — the same file then existed under two spellings, a query by the indexed one missed the new rows, and one run could count the file twice. The projects table records only the canonicalised root, so the caller's argument cannot be recovered; instead the spelling the file is already stored under is looked up and reused (the common spellings are enumerated explicitly — no suffix guessing, which could match a different file). Verified: a relative-rooted index holding `tmp_probe_rel/main.go` stayed at one identity after `index_file` was called with the absolute path (was 2 identities and 6 absolute-path rows), an absolute-rooted project is unchanged, and a not-yet-indexed file is still stored as given. Also found while verifying: `index_file` goes through `engine_index_file`, a different entry point from `engine_index_files` — the first version of this fix patched the wrong one, which the end-to-end check caught.
- **`detect_documentation_drift` no longer invents drift out of capability lists** (`verify/documentation_drift.cpp`): `extractLanguageClaims` counted every language mention anywhere in the README, so a "Supported Languages" matrix — and a benchmark table whose `Language` column describes *other* projects — was read as "this project is written in Python / Go / TypeScript / Java / …". On a self-index that produced five drifts, every one of them fabricated: a drift detector that drifts, which is the single failure that makes its output worthless. A mention now counts as a claim only when its context makes it one, judged from the mention's line, its markdown table's header, the nearest heading above it and its paragraph — capability statements ("Supports X", a `| Language | Parser |` matrix), third-party tables (a `Project` column) and per-language handling notes ("…are also top-only", "skipped", "rejected") are excluded, while "Written in …" and a Tech Stack row still count. A self-index now reports `claimed=["C++","Rust"], missing=[], drifts_found=0`. The test fixtures were rewritten rather than preserved: they had encoded the drifted behaviour (`"Supports C++, Python, Rust, and Go."` asserted Python and Go as project-language claims).
- **`C++` is no longer counted as two mentions** (`verify/documentation_drift.cpp`): `isWordBoundaryAfter` tests word characters, and `+` is not one, so the bare-`C` rule matched inside `C++` and the same occurrence was counted by both matchers. `C++`/`C#` are their own spellings now, which the mention counts and the new fixtures rely on.
- **`.kilo/` agent worktrees no longer participate in indexing** (`.gitignore`): `.kilo/worktrees/` holds full copies of this repository, so indexing it doubled every symbol (1515 of 3073 entities were the copy) and made by-name lookup ambiguous — `find_callers`/`find_callees` answered with `total: 0, ambiguous: true` plus a candidate list instead of the callers, `search` returned every hit twice and `get_communities` reported two identical communities. Git already ignored the directory through `.git/info/exclude`; listing it in `.gitignore` makes that visible to everyone and to the indexer's gitignore pass, which is what the walk actually reads. A self-index now yields 1833 entities instead of 3266.
- **`CODESCOPE_EXCLUDE_PATHS` was documented wrong, not implemented wrong** (`filter_policy_ignore.cpp`, `filter_policy_detect.cpp`, `scheduler/worker.rs`): the comments claimed `test/*` matches `test/sub/bar.py`, but patterns are matched against the whole relative path and `*` does not cross `/` — only `test/**` prunes a subtree, while a bare `test` or `test/` matches nothing. The suggested pattern set therefore excluded almost nothing. Comments and examples now use `**`, and the quarantine exclusion emits a single `**/<rel>` (one pattern covering the module-relative and project-rooted forms at any depth) instead of two depth-limited spellings.
- **C/C++ definitions with a qualified return type were invisible to the index** (`ir/translators/c_visitor.cpp`): `extractName` / `extractQualifiedName` scanned a node's children in order, and a `function_definition`'s first named child is its return type — parsed as a `qualified_identifier` for `std::string`. The scan matched the type, recursed into it, found no name, and returned "", which made the function handler skip the record. Every function returning `std::string` was therefore missing from the graph: on a self-index, `engine/src/query/**` contributed 1972 statements but only 3 definitions, so no `QueryEngine::` method existed and `find_symbol getCommunities` reported "not found" while its FFI export was indexed. Both helpers now read the `declarator` **field** instead of relying on child order (and handle the case where the declarator is already the name node, which otherwise dropped every free function). Function-definition statements rose 2672 → 3266 on the self-index, and `char *`, `std::string`, member, static and namespace-nested shapes are all covered; the accuracy gate stays at TP 36 / 0 FP / 0 FN.
- **A failed schema migration can no longer pass silently** (`store/store_schema_migrations.cpp`): the migration pass ran `exec("ALTER TABLE …")` at 34 sites without inspecting the result and returned `true` regardless, so a failed ALTER left a half-migrated database whose queries later failed with "no such column" — a failure reported far from its cause. Every ALTER now runs through a `migrationExec` wrapper that records the failure and prints the failing SQL, and the pass refuses to report a migrated schema. This covers all 34 call sites and any future one, without a column list to maintain; the per-site checks added for the `architecture_state` migration remain, failing fast with the specific cause where it is known.
- **`kConfidenceCgoCallback` is now `[[maybe_unused]]`** (`model/semantic_fact_extractor.cpp`): the constant is the only remaining reference to the intended `cgo_callback` rule, so it is deliberately kept rather than deleted — and marking it removes the last compiler warning from a clean build.
- **Normal cross-module dependencies are no longer reported as architecture violations** (`model/state_builder.cpp`, `model/project_state_builder.cpp`, `store/store_schema*.cpp`, `verify/dead_code_inspector.cpp`): `architecture_edge` is written per (caller module, callee module) pair with no layer model and no direction test — `layer_lower` / `layer_upper` hold module NAMES — yet `buildArchitectureState` counted those rows as `violations` with `compliance = 0.0`, penalising every project on the architecture score in proportion to how interconnected it is (a fixture with 11 cross-module calls scored as 11 violations). The count is real information and is kept, under a new `cross_module_edges` column: the migration MOVES the old `violations` value into it rather than discarding it and resets `compliance` to 1.0, so a pre-existing database ends up with the same honest numbers as a fresh one. The migration also collapses the duplicate `(project_id, layer)` rows the old non-idempotent rebuild accumulated (newest row wins — summing the copies would have inflated `cross_module_edges`), runs the correction unconditionally instead of only in the startup that adds the column (so a downgrade→upgrade cycle cannot strand pre-fix rows), and every step — probe, `ALTER`, dedup, correction — now fails the schema build loudly instead of silently skipping, per the file's own contract. `test_project_state` Tests 10–12 pin honesty, idempotency and the migration. `violations` stays 0 because nothing here can tell a violation from a dependency, and `project_state.architecture` now reports both (`{"score":1,"violations":0,"cross_module_edges":11}`). The same miscount made `DeadCodeInspector` emit a `LayerViolation` finding asserting "lower layer should not depend on upper layer" from two module names; it is now reported as `ModuleCoupling`. `buildArchitectureState` also rebuilds idempotently — its `INSERT OR IGNORE` had no unique key to ignore on, so every rebuild appended a duplicate copy of every row and doubled every count.
- **Read entry points now wait for the background knowledge builder** (`async_knowledge.{h,cpp}` plus 12 FFI files): the builder shares one sqlite3 connection with the caller and opens its own transactions, and only the WRITE entry points waited for it (`engine_index_project`, `engine_index_files`, the enhance path). A read issued while enrichment was still running could therefore interleave BEGIN/COMMIT on that connection and observe a half-built knowledge layer. Every read entry point now calls `waitForKnowledgeBuilder()` first — 63 guard sites. The initial sweep covered 52; a follow-up review mechanically diffed `engine.h`'s exports against the call sites and found 11 it had missed (`engine_search_semantic`, `engine_detect_changes`, `engine_get_communities`, `engine_export_artifact`, `engine_import_artifact`, `engine_find_connected_components`, `engine_trace_path`, `engine_explore_function`, `engine_detect_ffi_boundaries`, `engine_get_project_info`, `engine_get_verifier_registry_status`), now guarded too. This cannot self-join — the builder thread calls `runModelIndexSync()`/`buildKnowledgeGraphSync()` directly and never re-enters a read entry point — and the index paths are deliberately left unchanged. The wait is one-directional by design (option A): a read cannot *start* while the builder runs, but a builder launched by a later index call can still start during an in-flight read, and the join has no timeout — closing both directions needs a lock around the shared connection, recorded as still open.
- **`ok` now means the index run completed, not "some worker finished"** (`scheduler/mod.rs`, `scheduler/dynamic.rs`, `scheduler/chunked.rs`): `"ok": success > 0` ignored the merge result, so a run with a crashed module — or one whose module DBs never merged into main.db — still reported `ok: true` while the caller's view of the project was partial or empty. `ok` and a new `complete` field are both `run_complete(failed_workers, merged)`; `success` / `fail` / `merge` keep reporting the partial picture for callers that want it. The chunked path still drops a crashed/timed-out worker's DB without quarantine, so the run is now reported as incomplete but the files are not yet recovered.
- **Orphan-module detection never ran: its SQL called `reverse()`, which SQLite does not provide** (`verify/dead_code_inspector.cpp`): the statement failed to prepare on every call and the inspector silently returned an empty list, so "no orphan modules" was a permanent false negative for every project. Fixed with a `rtrim`-based basename extraction (`rtrim(X,'/')` drops trailing slashes; `rtrim(X, replace(X,'/',''))` strips trailing non-slash characters, so its length is the index of the last `/`), verified against `src/`, `src/foo/bar` and `pkg`. Four further silent `sqlite3_prepare_v2` failures in the same class now log the failing SQL with a `[module=verify, method=dead_code_inspector]` tag instead of being indistinguishable from "the inspection found nothing".
- **A lock-shaped name is no longer read as synchronisation evidence, and absence no longer contradicts** (`verify/contract_verifier.cpp`): the patterns were matched with `LIKE`, where `_` is a single-character wildcard, so `%_lock` matched `Block` ("B" + "lock") and a project containing a `Block` class was reported thread-safe at 0.7 confidence. `entitiesMatchingAny` now uses `ESCAPE '\'` and escapes `_` (identifiers contain real underscores, so `_` must be literal), the list is narrowed to actual primitives (`%mutex%`, `%atomic%`, `%spinlock%`, `%.lock`, `%lock_guard%`, …) instead of a bare `%lock%`, and the ZeroCopy check's `%view%` no longer matches `Review`/`Preview`/`Overview`. Absence of a match now returns **Unknown** instead of `Contradicted` — a pattern that fails to match is not evidence that the code is unsafe (Rust `std::sync`, Java `synchronized`, an OS lock or a message-passing design are all invisible to it), which is the treatment `verifyMemorySafe` already used.
- **Drift detectors no longer draw conclusions from an empty backend** (`verify/capability_drift.cpp`, `verify/documentation_drift.cpp`, `verify/dead_code_inspector.cpp`): with nothing indexed, `countImplementingEntities()` returns 0 for every capability, so each declared capability was reported as a severity-2 CapabilityDrift ("declared in the README but not implemented"); and `NOT EXISTS (SELECT 1 FROM import …)` is vacuously true with no imports, so every module with ≥10 entities was reported as an orphan. Capability drift now uses the same `evidence_backend_ready()` gate as the four registry verifiers; documentation drift requires `entity` rows only (it reads nothing else — requiring relations too suppressed the check for projects without resolvable call edges, which is what `test_documentation_drift` caught); the orphan check requires both `entity` and `import` rows.
- **Quarantine no longer drops healthy files on the strength of a filename** (`scheduler/worker.rs`, `scheduler/quarantine.rs`): `make_relative_glob` returned `*/{basename}`, so quarantining a crashing `a/foo.cpp` also excluded the healthy `c/foo.cpp`. The retry worker is rooted at the module dir, so the exclusion must carry the module-relative directory. The quarantine de-duplication had the same flaw — it compared basenames, so a same-named healthy file counted as already quarantined and was dropped from the retry without ever being tested. Both now use module-relative paths.
- **The CMake generator was chosen without regard to the existing build tree** (`Makefile`, `server/build.rs`, `engine/CMakeLists.txt`): Ninja was selected whenever `ninja` was on `PATH`, independently of how `engine/build` (or `engine/build-release*`) had actually been configured. On a tree generated with Unix Makefiles — e.g. ninja installed after the tree was created — the next `CMakeLists.txt` change made `make` run `cmake -G Ninja` against that tree, which fails with "Does not match the generator used previously"; the recipe piped the configure output through `tail`, whose exit status is always 0, so make counted the failed configure as a success and reused the stale build system. The symptom was silent and severe: newly added source files were never compiled — observed while adding `query/query_communities.cpp`, where `make` reported a successful engine build whose archive did not contain the new translation unit (the tests then failed to link `QueryEngine::getCommunities`). Both entry points now resolve the generator from the tree's own `CMakeCache.txt` and apply the "Ninja if available" heuristic only to a tree with no cache; an explicit `BUILD_GENERATOR=` that contradicts the tree fails with an actionable message (run `make clean`) instead of cmake's two-liner; and the Makefile's configure recipe runs under `set -o pipefail` so a failed configure aborts the build. `engine/CMakeLists.txt`'s `if(NOT CMAKE_GENERATOR)` block — which could never work, because CMake sets `CMAKE_GENERATOR` before the project file is read, making the `set(... CACHE ...)` a no-op — is replaced with a comment pointing at where the generator is actually selected.
- **Cross-language CALLS edges from the single-candidate fast path** (`resolver/pipeline.cpp`): when exactly one candidate existed in the caller's directory the fast path emitted the edge with `confidence=0.85` without applying the language hard filter that the main candidate loop applies, so a `.cpp` call site could resolve to a `.py` entity. The fast path now applies the same rule.
- **Every C call edge was dropped by the language hard filter** (`resolver/pipeline.cpp`, `resolver/factors.h`): the filter compared raw language labels, but the path-based classifier reports `.c` as `"cpp"` while the C visitor labels the translation unit `"c"`. The comparison now treats C/C++/ObjC as one language family, so `.c` call sites resolve again (the single-candidate fast path had been masking this for one-candidate references only). `languageFromPath` / `languagesCompatible` moved to `factors.h` as shared inline helpers, removing the per-TU copy that `pipeline_load.cpp` duplicated.
- **Non-deterministic candidate selection** (`resolver/pipeline_load.cpp`, `resolver/pipeline_apply.cpp`): the entity index was loaded with no `ORDER BY`, and the candidate sort compared only `total_score`, so equal-scoring homonyms were ordered by SQLite's scan order / `std::sort`'s unspecified ordering. The index is now ordered by `id` and ties break on `entity_id`, making the resolved edge reproducible.
- **`flushResolvedEdges` destroyed buildGraph's savepoint** (`resolver/pipeline_flush.cpp`): it opened a plain `BEGIN`/`COMMIT` while already inside `buildGraph`'s `SAVEPOINT` (and the caller's transaction). The `BEGIN` failed silently and the `COMMIT` committed the caller's transaction and released the savepoint, so a later failure could no longer roll the build back — the all-or-nothing guarantee was silently lost, and `RELEASE SAVEPOINT buildGraph` errored with "no such savepoint". It now uses a nested `SAVEPOINT flush_resolved_edges`, checks every write, and returns `false` so `ResolverPipeline::run()` returns -1 and `buildGraph` rolls back.
- **JS/TS recorded no structured call facts** (`ir/translators/js_visitor.{h,cpp}`): `visitCallExpr` never called `setCallFacts`, unlike the Go/Python/C/Rust/Java visitors, so `receiver_text`/`receiver_type`/`qualified_target`/`import_alias` were always empty for both languages. That disabled the resolver's fuzzy fallback (its `has_evidence` gate needs one of them) and left `factorReceiverTypeMatch` neutral for every candidate. The visitor now extracts `this`/object receivers, namespace/default/named import aliases, and records the facts; a bare call deliberately records nothing.
- **Module `entry` role could never fire** (`model/state_builder.cpp`): `entry_reachable` was read from the deprecated `graph_nodes.is_entry_point`, which has not been written since the canonical-schema migration, so the `'entry'` branch was dead and modules containing `main` were misclassified. It is now derived from `entity.name` + `entity.language`, mirroring `graph::isEntryPointName`.
- **Use-after-free in `createSchema`** (`store/store_schema.cpp`): the "create `type_info` if missing" block called `sqlite3_finalize` on the `probe` handle that the earlier `semantic_records` migration had already finalised, and leaked the `probe2` handle it had just prepared. Taken on every open of an existing database (the table is created by the main schema DDL, so the branch runs even on the first open).
- **`buildGraph` reported success on a partially written graph** (`store/store_graph.cpp`): the critical `INSERT ... SELECT` statements (entity, route, type_info, type_ref, reference, scope) ignored `exec()`'s return value, so a failure (schema drift, `SQLITE_BUSY`, disk full) still reached `RELEASE SAVEPOINT` and returned true. They are now checked through an `exec_write` helper that accumulates failure and forces a rollback + false return; `flushImportBatch` returns a status instead of only logging.
- **`insertFileResultBatch` ignored `sqlite3_step` results** (`store/store_batch.cpp`): a failed `INSERT INTO files` / `file_scan_state` or a failed `DELETE FROM semantic_records` was only logged, so a failed DELETE left the previous run's rows behind for the plain INSERT to append duplicates to (which `buildGraph` turns into duplicate entities). Each step result is now checked and the batch fails.
- **In-memory bulk index path could abort the process** (`engine_index_project_membulk.cpp`): the default path for modules up to 2000 files had no `try`/`catch` around the visitor/translator (the streaming path does), so any malformed file terminated the whole index via `std::terminate`. It also used an uninitialised `struct stat` when `stat()` failed, writing garbage into the incremental-index freshness baseline, and silently skipped stat/read/language/tree failures instead of recording them via `bufferParseFailure`.
- **Background enrichment raced the index on one SQLite connection** (`async_knowledge.cpp`, `engine_index_project.cpp`, `engine_index_files.cpp`, `engine_queries.cpp`): the async builder opens its own transactions on the shared `g_store` connection, so a concurrent index/`enhance` could interleave `BEGIN`/`COMMIT` ("cannot start a transaction within a transaction") or commit the other side's half-written state. The write entry points now wait for the builder (`joinAsyncKnowledgeBuilder()`, a no-op once it has finished).
- **MCP arguments were not clamped** (`server/src/tools/mod.rs`): `depth` reached the engine without an upper bound, and `radius`/`limit`/`edge_type` used `as i32` directly, so a large JSON number was truncated into an arbitrary (sometimes negative) value. `depth` could recurse until the stack was exhausted and abort the long-running server. All four are now clamped by named helpers with boundary unit tests.
- **Chunked scheduler silently dropped files** (`server/src/scheduler/mod.rs`): a plan with more chunks than the fixed-size shared-memory queue held was truncated with only a stderr warning — the trailing files were never indexed while the result still reported `ok=true`. It now fails with an explicit error instead of producing an incomplete index.
- **Entity ids depended on the row insertion order** (`store/store_graph.cpp`): `ROW_NUMBER() OVER ()` numbered declarations in `semantic_records` scan order, which follows the INSERT order and therefore the parse-worker scheduling, so the same input could produce different entity ids (and every id-derived row — relation, type_ref, import, CSR — shifted with it). The window is now ordered on the record's semantic identity (`file_path, kind, start_row, start_col, original_id`), making the assignment a pure function of the input instead of a property of whatever query plan SQLite happens to pick.
- **Batch type inserts committed a failed batch** (`store/store_type.cpp`): `insertTypeInfoBatch` / `insertTypeRefBatch` opened a plain `BEGIN`, called `commitTransaction()` on a prepare failure (committing the caller's transaction) and returned `true` even when a batch `step()` failed. Both now use a nested `SAVEPOINT`, roll back on any failure, and report `false`.
- **`cleanupStaleFiles` opened a nested transaction and ignored its results** (`store/store_project.cpp`): `BEGIN IMMEDIATE` inside the index transaction fails silently, and the trailing `COMMIT` then committed the caller's transaction; the prepared-INSERT and DELETE steps were also unchecked. It now uses a `SAVEPOINT` and checks every step, rolling back on failure.
- **`buildVectorsFromGraph` could leave the connection inside a transaction** (`store/store_search.cpp`): `BEGIN IMMEDIATE`/`COMMIT` were unchecked and the loop had no exception safety, so a throwing allocation (or a failed commit) skipped the `COMMIT` and left the connection in a transaction — every later `BEGIN` in the process would then fail. It now uses a `SAVEPOINT` with an RAII guard that rolls back unless released, and aborts the batch when a vector insert fails.
- **`insertSemanticRecordsBatch` reported nothing on failure** (`store/store_batch.cpp`): a prepare or `step` failure was only logged (or stored in `error_`) while the function returned `void`, so callers could not detect a lost batch. It now returns `bool` and reports the failing batch.
- **`buildKnowledgeGraphSync` left the transaction open when `COMMIT` failed** (`async_knowledge.cpp`): the error path returned without rolling back. It now issues `ROLLBACK` so a failed commit cannot make every later `BEGIN` fail until the process restarts.
- **Builtin-name filtering silently dropped user-defined methods** (`go_visitor.cpp`, `java_visitor.cpp`, `python_visitor.cpp`, `js_visitor.cpp`): each visitor extracted the callee as the last selector/attribute segment and then filtered it against its builtin list, so a qualified call whose method name collided produced **no call record at all** — a false negative the graph cannot distinguish from "no such call". The collisions were not exotic: Java listed `map`, `filter`, `forEach`, `collect`, `reduce`, `indexOf`, `replace`, `format`, `compareTo`, `startsWith`, `equals`, `toString`, `hashCode`, `clone`; Go added `copy`, `close`, `delete`, `new`, `len`, `min`, `max`; Python `format`, `next`, `set`, `map`, `filter`. The filter now applies only to **unqualified** calls (a call with a receiver can never be a pre-declared identifier, and the visitors already carry receiver/import evidence for it). Rust already matched on the full qualified text, so it was unaffected. C/C++ keeps the old behaviour on purpose — its list is stdlib names (`malloc`/`free`/`memcpy`/`printf`) and `ops->free(x)` is an idiomatic vtable field call with no receiver-type evidence, so emitting it would re-open the false-positive flood the filter exists to prevent.
- **Capability matching accepted coincidental prefixes** (`verify/capability_verifier.cpp`, `verify/capability_drift.cpp`): entity/capability names were matched with a bidirectional `LIKE || '%'` and no length floor, so for capability "GetNeighbors" any 1-3 character symbol (`get`, a single-letter variable) satisfied the reverse direction and was counted as implementing the capability. Prefix matching now requires both sides to be at least 4 characters (clearing the common `get`/`set`/`add`/`map`/`run`/`i`/`x` symbols); exact equality still matches at any length.
- **Ordinary business directories were skipped at any depth** (`filter_policy.cpp`): `scripts`, `hack`, `migrations`, `seeds`, `integration`, `locale(s)`, `i18n`, `l10n`, `assets`, `static`, `public`, `media` and `external` were in the any-depth skip set. Outside the web-frontend convention these are first-party directories (`src/integration/`, `pkg/scripts/`, `db/migrations/`, `app/static/`), so the skip silently dropped source files — symbols and edges that the index then reported as non-existent. They are now skipped only within the first three path components, where those conventions actually live; the non-source formats filling them remain rejected by the extension filter. `test`/`tests`/`docs`/`examples`/`samples`/`e2e`/`vendor`/`bench*` stay any-depth (documented policy, Java carve-out unchanged), and the previously dead `top_only_skip_dirs_` member is now the set that drives this layer for every language.

### 🔧 Improvements

- **The accuracy gate no longer under-reports its own coverage — asserted call edges 33 → 36** (`engine/tests/accuracy/fixtures/java/ground_truth.json`, `engine/tests/accuracy/fixtures/ts/ground_truth.json`): the Java `b.run()` and TypeScript `obj.render()` / `logger.log()` edges were filed under `allowed_unresolved` ("receiver type not yet propagated" / "cross-method resolution still in progress"), but the resolver has been resolving all three for some time — `allowed_unresolved` only *tolerates* an edge if it appears, so the gate was neither requiring the behavior nor able to catch a regression on those paths. All three are now `expected_calls`. The stale Java `mainFunc → bravo` tolerance entry is deleted: the fixture contains no such call site, and as a tolerant entry it would have silently absorbed a false positive `mainFunc → bravo` edge. `make accuracy-check` stays green with P/R/F1 = 1.0 (0 FP / 0 FN) at TP 36, and the Java fixture comment now matches the asserted edges.
- **The test gate is derived from the sources, not from hand-maintained lists** (`Makefile`, `.github/workflows/_ci.yml`): `TEST_EXES` is now `engine/tests/*.cpp` minus an (empty, documented) `TEST_EXCLUDES`, and CI runs every built test whose source still exists. The previous lists had drifted in both directions: `make test-engine` omitted six passing tests, and CI skipped 26 passing tests — including all eight `test_fp_*` false-positive precision tests and the six multi-language e2e tests — for reasons that were no longer true ("known failing", "requires removed LadybugDB graph feature"). The CI list is now empty, with a policy comment forbidding new entries that hide failures.
- **`make test-engine` warns when the build tree is not Debug** (`Makefile`): `assert()` is compiled out under `-DNDEBUG`, which makes assert-based tests vacuous; the warning points at the reconfigure command. CI already builds tests with `CMAKE_BUILD_TYPE=Debug`.
- **New regression tests**: `test_index_determinism` (the same fixture indexed twice — with 1 and with 8 parse workers — must produce identical entity ids and relations, plus a controlled-order check that inserts the same two records forward and reversed and requires the ids to follow the semantic key), `test_resolver_language_filter` (cross-language candidate rejected; C-family candidate accepted), `test_js_ts_call_facts` (`ns.fn()`, `obj.method()`, `this.method()`, and the bare-call boundary case), `test_schema_reopen` (reopen cycles keep the schema intact and leak no prepared statements), plus four `clamp_*` unit tests in `server/src/tools/mod.rs`. The `test_schema_reopen` and `test_index_determinism` assertions were both verified to fail against the pre-fix code. This round adds `test_builtin_method_calls` (a colliding method call in Go/Java/Python/JS must keep its record, while a bare builtin call stays filtered — plus the deliberate C exception), `test_capability_prefix_floor` (3- and 1-character symbols must not match a capability by prefix; exact equality still matches), and `test_filter_policy_depth` (business-plausible dirs are skipped only within the first three path components; `tests`/`vendor`/`node_modules` stay any-depth; the Java carve-out is re-checked). All three were verified to fail against the pre-fix code.

### 🧹 Chores

- Split `resolver/pipeline.cpp` helpers into `resolver/factors.h` to stay under the 1000-line rule after the fixes.
- **Split every source file that exceeded the 1000-line rule.** Each split is pure code motion (no logic change), verified by the full suite after each step. New TUs are registered in `engine/CMakeLists.txt`, which lists sources explicitly rather than globbing.
  - `store/store_graph.cpp` 1212 → **854**: import batch writer (`PathRec` + `flushImportBatch`) → `store/store_graph_imports.{h,cpp}` (57); BLOB-packed call-edge CSR (`buildCSR`, `decodeAdjacencyBlob`, `getCalleeIds`, `getCallerIds`) → `store/store_graph_csr.cpp` (327).
  - `store/store_schema.cpp` 1760 → **938**: the per-column migrations that patch pre-existing databases → `store/store_schema_migrations.cpp` (`GraphStore::runSchemaMigrations`, 856), declared private in `store.h`. `createSchema()` now returns `ok && migrations_ok`, which is exactly equivalent to the old control flow (a hard migration failure returned `false`; otherwise the result was the main DDL's `ok`).
  - `store/store_search.cpp` 1207 → **668**: the n-gram hashing producer/consumer (`buildVectorsFromGraph`, `searchSemanticJson`, and the `kVecDim`/`kSemanticScoreFloor`/`hashMix` constants) → `store/store_search_vector.cpp` (523). `splitIdentifierWords` is used by both halves and moved into the shared `store/store_search_words.h`.
  - `engine_queries.cpp` 1462 → **774**: path tracing, interactive function exploration, context building and FFI-boundary detection → `engine_queries_context.cpp` (709). These are pure C ABI export points declared on the Rust side, so moving the definitions needs no header change.
  - `server/src/tools/mod.rs` 2080 → **852**: the tool catalog (`all_tools`, 522) → `tools/catalog.rs`; the indexing/worker machinery plus the worker timeout/retry constants that only it uses (543) → `tools/indexing.rs`; the quick project scan (221) → `tools/discover.rs`.
  - `server/src/scheduler/mod.rs` 1512 → **643**: the opt-in chunk-level scheduler (376) → `scheduler/chunked.rs`; the retired shm-core-pool scheduler kept for reference plus `get_child_pids` (524) → `scheduler/dynamic.rs`. Both use `use super::*` on purpose — as children of `scheduler` they inherit the parent's private helpers without widening their visibility crate-wide.

  A second pass then cleared the **9 remaining offenders** the first audit had not listed (the original list was stale — two of them, `go_visitor.cpp` and `filter_policy.cpp`, were already over 1000 lines before any change this session):
  - `engine/src/ir/builtin_registry.cpp` 1826 → **633**: the symbol tables are pure data, so they split by language family into `builtin_registry_systems.cpp` (C/C++/Rust/Go, 673) and `builtin_registry_app.cpp` (Java/JS/Swift, 560), unified by the new private `ir/builtin_registry_tables.h`. The accessors had to lose their TU-local `static` for the dispatch table to reach them across TUs; each is still a function-local `static` set, so a project pays only for the languages it contains. (`builtin_registry.cpp` keeps Python + the dispatch table + the public API.)
  - `engine/src/engine_ffi.cpp` 1680 → **836**: knowledge-graph direct query + navigation → `engine_ffi_graph.cpp` (467); batch indexing + project metadata → `engine_ffi_index.cpp` (421). Their static helpers (`appendFindingJson`, `detectLicense`) moved with their only caller.
  - `engine/src/query/query_engine.cpp` 1431 → **848**: the Step-7 entity-precise caller/callee APIs plus neighbour/shortest-path/subgraph walks → `query_engine_traverse.cpp` (621), along with `kShortestPathMaxDepth` / `kShortestPathNote`, which only `findShortestPath` reads.
  - `engine/src/ir/translators/go_visitor.cpp` 1285 → **700**: `handleTypeDecl` + `handleCall` → `go_visitor_calls.cpp` (606), together with `kHttpMethods` and `isGoBuiltin`, the two TU-local helpers only `handleCall` uses.
  - `engine/src/engine_verify_ffi.cpp` 1235 → **837**: `engine_explain_module` (the module Knowledge Card, the largest single export here) → `engine_verify_explain.cpp` (445), with its scoring constants duplicated rather than shared — a shared header would let a change to one export's scoring silently change another's.
  - `engine/src/filter_policy.cpp` 1175 → **846**: language detection (extension + shebang probe), component-wise path matching, gitignore-style rule matching and the loader → `filter_policy_detect.cpp` (347).
  - `engine/src/store/store_query.cpp` 1007 → **645**: path tracing + interactive function exploration → `store_query_explore.cpp` (392).
  - `server/src/scheduler/merge.rs` 1352 → **873**: split by responsibility rather than by size — `merge_module_dbs` is the entry point the scheduler calls and `fetch_all_columns_excluding_rowid` discovers each unit DB's column list, so they became `merge_driver.rs` (418) and `merge_fetch.rs` (102) as children of `merge`. The driver is `pub(crate)` because `merge.rs` re-exports it one level up: a re-export can never widen beyond the item's own visibility.
  - `server/src/scheduler/chunk_queue.rs` 1034 → **795**: claim/mark-done/stale-recovery/counters → `chunk_queue_ops.rs` (259), a second `impl ChunkQueue` block beside the mmap setup/teardown in the parent.

  **`engine/src` + `server/src` now contain zero files over 1000 lines.**
